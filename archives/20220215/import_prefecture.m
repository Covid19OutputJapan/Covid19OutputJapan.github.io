...
covid = importdata([dropbox_path 'Covid_weekly_NewV.csv']); % Import weekly Covid data by prefecture
...
dateD = Data(:, 1) + 21916;
N = Data(:, 2); %New cases
dD = Data(:, 3); %New deaths
M = Data(:, 4); %Mobility
% GDP = Data(:,5); 
POP = Data(:, 6); %Population
GDP = Data(:, 7); 
...
BED = zeros(Tdata, 1); %Number of beds for ICU_nation
ICU_nation(2:Tdata + 1, 1) = Data(:, 24);
BED(1:Tdata, 1) = Data(:, 25);
ICU_pref(2:Tdata + 1, 1) = Data(:, 31); %Data(:, 21);
%--- Import hospitalized patient data ---%
hospital = zeros(Tdata + 1, 1);
hospital(2:end) = Data(:, 32);
hospital(isnan(hospital)) = 0;
%--- Import SIRD data ---%
I_data = zeros(Tdata + 1, 1);
I_data(2:end) = Data(:, 28);
R_data = zeros(Tdata + 1, 1);
R_data(2:end) = Data(:, 29);
D_data = zeros(Tdata + 1, 1);
D_data(2:end) = Data(:, 30);
