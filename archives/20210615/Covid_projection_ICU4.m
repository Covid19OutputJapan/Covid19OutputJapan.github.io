function [CumD,GDPLoss,alphapath,SimData,SimN,SimERN,THonPath,SimICU,beta] = ...
    Covid_projection_ICU4(InitialValues,alpha_on,alpha_off,th_on1,th_on2,...
    th_off1,th_off2,th_off3,beta,gamma,delta,delta_init,V,h,k,POP0,...
    hconstant,alpha_duration,state,new_ICU,gamma_ICU,...
    beta_shock_after_emergency,rho_after_emergency)
...
%alpha_off_path = alpha_on+(alpha_off-alpha_on)/2:(1/2)*((alpha_off-alpha_on)/(alpha_duration+1)):alpha_off;
counter = 1;
wave = 0;
wave_off = 0;
th_on1_ori = th_on1;
...
%     end
    ...
    SimICU(i+1) = max(0, SimICU(i) + new_ICU*delta(i)*SimN(i) - gamma_ICU*SimICU(i) - delta(i)*SimData(i,2));
    if th_on <= SimN(i)
        alpha = alpha_on;
        beta_shock = beta_shock_after_emergency;
        counter = 1;
