...
% 緊急事態宣言の発令基準
th_on_vector = [1000,500,400,350,1000,350,350]; % present.
th_on_vector2 = [1500,1000,800,700,1500,350,350]; % 高齢者がうち終わったあとの基準

% 緊急事態宣言の解除基準
th_off_vector = [350,100,110,80,100,50,60]; %1回目の緊急事態宣言の解除基準
th_off_vector2 = [350,100,80,60,100,50,60]; %2回目の緊急事態宣言の解除基準
th_off_vector3 = [350,100,80,60,100,50,60]; %3回目の緊急事態宣言の解除基準

% 緊急事態宣言の強さの基準: Simulation開始時のERNをいくつにするか
alpha_on_vector = [0.2,0.1,0.1,0.1,0.2];
% Size of AR(1) shock for beta process
betaT_temp_ini_vec = [-0.1,0,0,0,-0.5,0,0,0];%-0.1
start_beta_vec = [1,1,1,1,1];
beta_rho_vec = [0,0,0,0,0];
...
var_ss_vector = [0.9, 1, 1, 1, 0.90, 0, 0, 0,0]; % 5/17
share_index = 2; % = 2 for Monday, = 1 after Wednesday

% Parameters for Vaccine Path
paces_ori_vec = [5250000,0,0,0,5600000];
gradual_paces_vec = [1,0,0,0,1];
sw_vacpath_vec = [0,0,0,0,0];
VT3share_vec = [0.5,0,0,0,0.5];
lag_vec = [3,0,0,0,3];
medical_duration_vec = [1,0,0,0,1];

ind_date_vec = [datetime(2021,7,15),datetime(2021,7,15),datetime(2021,7,15),datetime(2021,7,15),datetime(2021,7,15)];