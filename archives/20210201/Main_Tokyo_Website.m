...
for l = 1:2
    for y = 1:length(altA)
        ...
        for i = 1:length(TH)
            ...
            if TH(i) == TH_index(1)
                plot(NPath(:,i)/7,'-r','LineWidth',2,'DisplayName',sprintf('%.0f',TH(i)/7))
                if y == 1;
                red = NPath(:,i);
                end
            elseif TH(i) == TH_index(2)
                plot(NPath(:,i)/7,'-b','LineWidth',2,'DisplayName',sprintf('%.0f',TH(i)/7))
                if y == 1;
                blue = NPath(:,i);
                end
            else
                plot(NPath(:,i)/7,'--','LineWidth',0.3,'DisplayName',sprintf('%.0f',TH(i)/7))
            end
            hold on
        end
        ax = gca;
...
%--- Save data ---%
if data_save == 1
    writematrix([dateP(1:end-1)',red,blue],'NewCasePath.xls');
    writematrix(AD1,'GDPDeath0.xls');
    writematrix(AD2,'GDPDeath1.xls');
end
