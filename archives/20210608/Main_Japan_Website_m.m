...
close all
iPC= 1;
home = 'C:\Users\kenic\Dropbox\fujii_nakata\Website\Covid19OutputJapan.github.io\archives\20210608\';
% if iPC == 1
...
%ind_date = find(date_long == '5-Aug-2021');
ind_date = find(date_long == datetime(2021,08,05));
...
[S,I,R,D,GDP]...
    = SIRD(Tdata,POP0,N,E1,E2,V1_elderly,V1_medical,V2_elderly,V2_medical,gamma,dD,TdataGDP,referenceGDP,alpha,GDP);
...
if Vsimple == 0
    ...
    %     [V,deltaT,VT] = ...
    %         vaccine_distribution_medical_ori(vacpath,medical,...
    %         V1_medical,V2_medical,elderly,V1_elderly,V2_elderly, ...
    %         ordinary,elderly_total,delta_average,E1,E2,D1,D2,ps,POP0,...
    %         lag,medical_duration);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if iPC==1
  NextD = load('C:\Users\kenic\Dropbox\fujii_nakata\Website\Codes\Japan20210615.mat');
  FE = load('C:\Users\kenic\Dropbox\fujii_nakata\Website\Codes\Forecast.mat');
else
...
if dNForecast(end) ~= dNForecast(end-1) && length(dNForecast) == NextD.Tdata
  if iPC==1
      save('C:\Users\kenic\Dropbox\fujii_nakata\Website\Codes\Forecast.mat','dDForecast','dNForecast','ICUForecast','-append');
  else
...
AlphaIndexVariables = zeros(SimPeriod,7,length(AlphaVals));    % matrix to contain the projected variables for certain alpha values
...
for i = 1:length(TargetAlpha)
    ...
    [CumD(i),AverageAlpha(i),SimData,SimN, SimICU]= Covid_projection2(InitialValues,alphaT,betaT,gammaT,deltaT,V,h,k,POP0,hconstant,ICU_inflow_avg,gamma_ICU,delta_ICU);
...
for i = 1:length(AlphaVals)
    ...
    [CD,AA,SimData,SimN,SimICU] = Covid_projection2(InitialValues,alphaT,betaT,gammaT,deltaT,V,h,k,POP0,hconstant,ICU_inflow_avg,gamma_ICU,delta_ICU);
    AlphaIndexVariables(:,:,i) = [SimData(2:end,:),SimN,SimGDP,SimICU(2:end)];
end
%%
Past = [N,GDP,ERN,zeros(Tdata,1),ICU(2:end)];
VarList2 = ["N","Y","ERN","VShots","ICU"];
VarName2 = ["New infections","Output (deviation from the reference path)","Effective reproduction number","Newly vaccinated persons","Intensive Care Unit"];
VarName2_jp = ["新規感染者数","経済活動","実効再生産数","ワクチン接種","重症者数"];
...
for l = 1:2
    ...
    for j = 1:length(VarList2)
        ...
        for i = 1:length(AlphaVals)
            ...
            ProjectedData = [AlphaIndexVariables(:,5,i),AlphaIndexVariables(:,6,i),ERNT,VShots,AlphaIndexVariables(:,7,i)];
...
elseif iDrawUB==1
    ...
    for i = 1:length(TargetAlpha)
        ...
        parfor j = 1:Nsim
            ...
            [V,deltaS,VT,delta_ICU] = ...
                vaccine_distribution(V1_medical,V2_medical,...
                V1_elderly,V2_elderly,ind_date,VT3share,...
                elderly,elderly_total,medical,ordinary,delta_averageS,lag,...
                medical_duration,paces_ori,sw_vacpath,gradual_paces,...
                E1,E2,D1,D2,ps,POP0,SimPeriod,Tdata);
            ...
            %[V,deltaS,VT] = vaccine_distribution_medical_ori(vacpath,medical,V1_medical,V2_medical,elderly,V1_elderly,V2_elderly, ordinary,elderly_total,delta_averageS,E1,E2,D1,D2,ps,POP0,lag,medical_duration);
            %
            %             delta_ssS = delta_averageS*(0.09/1.27);
            %             deltaS = delta_averageS*ones(SimPeriod,1);
            %             deltaS(VacStart+3:VacStart+3+9+13) = delta_averageS:(delta_ssS-delta_averageS)/22:delta_ssS;
            ...
            HS=[HS1,HS2];
            [UB2(j,i)]= Covid_projection2(InitialValues,alphaT,betaS,gammaT,deltaS,V,HS,k,POP0,hconstant,ICU_inflow_avg,gamma_ICU,delta_ICU)
        end
    end
