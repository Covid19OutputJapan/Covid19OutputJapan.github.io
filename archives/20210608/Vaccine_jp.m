...
[V1_medical, V2_medical] = vaccine_daily_to_weekly_table(vaccine_medical, 1, dateEN,iPC);
[V1_elderly, V2_elderly] = vaccine_daily_to_weekly_table(vaccine_elderly, 1, dateEN,iPC);
...
% ==================== CHANGE HERE ====================== %
% ======================================================= %
xline_ind = find(date == '17-Jun-2021');
TH = [2800000,4200000,5600000,7000000];
% TH = [4200000,5600000,7000000];
TH_index = TH;
gradual_paces_vector = [0,1,2,3];
% gradual_paces_vector = [1,2,3];
sw_vacpath_vec = [0,0,0,0];
% sw_vacpath_vec = [0,0,0];
ind_date_vec = [find(date == '16-Sep-2021'),find(date == '5-Aug-2021'),find(date == '8-Jul-2021'),find(date == '1-Jul-2021')]; %Date by which the first shots should finish
% ind_date_vec = [find(date == '5-Aug-2021'),find(date == '8-Jul-2021'),find(date == '1-Jul-2021')]; %Date by which the first shots should finish
VT3share_vec = [0.5,0.55,0.5,0.55];
% VT3share_vec = [0.55,0.5,0.55];
lag_vec = [4,3,3,3];
% lag_vec = [3,3,3];
medical_duration_vec = [2,2,2,2];
% medical_duration_vec = [2,2,2];
...
VTMat = zeros(SimPeriod+1,6,nTH);
VTMat(1,1,:) = sum(V1_elderly);
VTMat(1,2,:) = sum(V2_elderly);
VTMat(1,5,:) = sum(V1_medical);
VTMat(1,6,:) = sum(V2_medical);
...
for iTH = 1:length(TH)
    ...
    paces_ori = TH(iTH);
    disp(paces_ori);
    gradual_paces = gradual_paces_vector(iTH);
    sw_vacpath = sw_vacpath_vec(iTH);
    ind_date = ind_date_vec(iTH);
    VT3share = VT3share_vec(iTH);
    lag = lag_vec(iTH);
    medical_duration = medical_duration_vec(iTH);

    %%%%%%%%%%%%%%%%% Figure Titles %%%%%%%%%%%%%%%%%%%%%%%%%
    figname_main = 'VP_MainResults_';
    figname_beta = 'BetaPath';
    figname_beta_loop = ['BetaPath' '_' sprintf(ft,paces_ori)];
    figname_beta_tilde_loop = ['BetaTildePath_', sprintf(ft,paces_ori)];
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    share = ((1 - sum(VD_elderly(1:end-2))/elderly_total) * (delta_average - delta_ss) ...
        + (1-sum(VD_medical)/(ordinary_total))*delta_ss)/delta_average;
    ...
    [V,deltaT,VT] = ...
        vaccine_distribution_special(V1_medical,V2_medical,...
...
%     disp(sum(sum(VT(1:find(date == '29-Jul-2021')-Tdata,:))/ps))
    VTMat(2:end,:,iTH) = VT;
...
%             plot_deltapath(201,delta,deltaT,deltaT(1),MonthWeekJP,WeekNumber,Tdata,fs,fn,iTH);
    ...
    elseif vaccine_figure_loop == 1
        plot_vaccinepath_ym(200+iTH,VT,V1_medical,V2_medical,V1_elderly,V2_elderly,date,ps,MonthWeekJP,WeekNumber,Tdata,fs,ldfs,fn);
        subplot(1,2,1)
        title(string(['新規ワクチン接種本数（週ごと）（',sprintf(ft,TH(iTH)),'）']), 'FontSize',fs,'FontName',fn);
        xline(Tdata+9,'HandleVisibility','off')
        xline(Tdata+13,'HandleVisibility','off')
        subplot(1,2,2)
        grid on
        title(string(['累計ワクチン接種本数（週ごと）（',sprintf(ft,TH(iTH)),'）']), 'FontSize',fs,'FontName',fn);
        xline(Tdata+9,'HandleVisibility','off')
        xline(Tdata+13,'HandleVisibility','off')
        xline(find(date == '30-Sep-2021'),'HandleVisibility','off')
        xline(find(date == '28-Oct-2021'),'HandleVisibility','off')
        xline(find(date == '25-Nov-2021'),'HandleVisibility','off')
...
%         title("致死率（現在のレベルで標準化）", 'FontSize',fs,'FontName',fn);
    ...
    elderly_vaccine1 = VT(:,1);
    elderly_vaccine2 = VT(:,2);
    disp('First Elderly Vaccination at the end of July ');
    disp((sum(V1_elderly) + sum(elderly_vaccine1(1:9)))/ps);
    disp('First Elderly Vaccination at the end of August');
    disp((sum(V1_elderly) +sum(elderly_vaccine1(1:13)))/ps);
    disp('Second Elderly Vaccination at the end of July');
    disp((sum(V2_elderly) + sum(elderly_vaccine2(1:9)))/ps);
    disp('Second Elderly Vaccination at the end of August');
    disp((sum(V2_elderly) +sum(elderly_vaccine2(1:13)))/ps);
    
    disp('First Elderly Vaccination at the end of September');
    disp((sum(V1_elderly) + sum(elderly_vaccine1(1:find(date == '30-Sep-2021')-Tdata)))/ps);
    disp('Second Elderly Vaccination at the end of September');
    disp((sum(V2_elderly) + sum(elderly_vaccine2(1:find(date == '30-Sep-2021')-Tdata)))/ps);   
    disp('First Elderly Vaccination at the end of October');
    disp((sum(V1_elderly) +sum(elderly_vaccine1(1:find(date == '28-Oct-2021')-Tdata)))/ps);
    disp('Second Elderly Vaccination at the end of Ocober');
    disp((sum(V2_elderly) +sum(elderly_vaccine2(1:find(date == '28-Oct-2021')-Tdata)))/ps);
   disp('First Elderly Vaccination at the end of November');
    disp((sum(V1_elderly) +sum(elderly_vaccine1(1:find(date == '25-Nov-2021')-Tdata)))/ps);
    disp('Second Elderly Vaccination at the end of November');
    disp((sum(V2_elderly) +sum(elderly_vaccine2(1:find(date == '25-Nov-2021')-Tdata)))/ps);
    if figure_save == 1
        saveas(figure(200+iTH), [home 'Figures/'  'Vaccine_path_' num2str(paces_ori) '.png']);
    end

    if figure_save == 1 && iTH == 2
    ...
    [DMat(iTH),AlphaMat(iTH),AlphaPath(:,iTH),SimData(:,:,iTH),NPath(:,iTH),SimERN(:,iTH),THonPath(:,iTH),SimICU(:,iTH),betaShock] ...
        ...
        betaT,gammaT,deltaT,deltaT(1),V,h,k,POP0,hconstant,...
...
%     subplot(2,2,1)
%     [BackDataN,BackDataAlpha,BackDataERN] = plot_SimN2(TH,TH_index,N,NPath,alpha,AlphaPath,ERN,SimERN,THonPath,MonthWeekEN,MonthWeekJP,WeekNumber,Tdata,linecolor,20,fs,fn,ft,l);
...
% 
%     if figure_save == 1
%         saveas(f,[home 'Figures/' char(pref) '/' char(figname_main) '.png']);
%     end
...
%     writetable(TN,[home 'Figures/' char(pref) '/BackData_' char(figname_main) '_' char(pref)  '.xls'],'Sheet','新規感染者数（1日平均）','WriteVariableNames',false);
%     writetable(TAD,[home 'Figures/' char(pref) '/BackData_' char(figname_main) '_' char(pref) '.xls'],'Sheet','経済損失と死亡者数','WriteVariableNames',false);
% end
% 
%%
week5 = find(WeekNumber == 5);
week4 = find(WeekNumber == 4);
Week5m1 = week5-1;
week4_true = setdiff(week4,Week5m1);
endmonth = union(week4_true, week5);
endmonth(1:find(endmonth <= Tdata,1,'last')) = [];
if vac_data_save == 1
    VTMat_jp = VTMat/ps;
    nM = length(VTMat(1,:,1));
    cumVTMat = cumsum(VTMat_jp,1);
    idx = [5 6 1 2 3 4];
    B = cumVTMat(:, idx, :);
    titleN = strings(1,1+nTH*6);
    titleN(1) = "週";
    week = [MonthWeekJP(Tdata);MonthWeekJP(endmonth)];
    firstcolumns = [round(B(1,1:nM,1)); squeeze(round(B(endmonth-Tdata+1,1:nM,1)))];
    secondcolumns = [round(B(1,1:nM,2)); squeeze(round(B(endmonth-Tdata+1,1:nM,2)))];
    thirdcolumns = [round(B(1,1:nM,3)); squeeze(round(B(endmonth-Tdata+1,1:nM,3)))];
    forthcolumns = [round(B(1,1:nM,4)); squeeze(round(B(endmonth-Tdata+1,1:nM,4)))];
    for ti = 1:nTH
        titleN(1,2+nM*(ti-1)) = string(['医療従事者1本目（',sprintf('%.0f',TH(ti)),'）']);
        titleN(1,3+nM*(ti-1)) = string(['医療従事者2本目（',sprintf('%.0f',TH(ti)),'）']);
        titleN(1,4+nM*(ti-1)) = string(['高齢者1本目（',sprintf('%.0f',TH(ti)),'）']);
        titleN(1,5+nM*(ti-1)) = string(['高齢者2本目（',sprintf('%.0f',TH(ti)),'）']);
        titleN(1,6+nM*(ti-1)) = string(['一般1本目（',sprintf('%.0f',TH(ti)),'）']);
        titleN(1,7+nM*(ti-1)) = string(['一般2本目（',sprintf('%.0f',TH(ti)),'）']);
%         titleN(1,2+nTH*(ti-1)) = string(['医療従事者1本目（',sprintf('%.0f',TH(ti)),'）']);
%         titleN(1,3+nTH*(ti-1)) = string(['医療従事者2本目（',sprintf('%.0f',TH(ti)),'）']);
%         titleN(1,4+nTH*(ti-1)) = string(['高齢者1本目（',sprintf('%.0f',TH(ti)),'）']);
%         titleN(1,5+nTH*(ti-1)) = string(['高齢者2本目（',sprintf('%.0f',TH(ti)),'）']);
%         titleN(1,6+nTH*(ti-1)) = string(['一般1本目（',sprintf('%.0f',TH(ti)),'）']);
%         titleN(1,7+nTH*(ti-1)) = string(['一般2本目（',sprintf('%.0f',TH(ti)),'）']);
    end
    TN = table([titleN;week,firstcolumns,secondcolumns,thirdcolumns,forthcolumns]);
    writetable(TN,[home 'Figures/VaccineData.xls'],'Sheet','ワクチン見通し','WriteVariableNames',false);
end
% %%
...
% subplot(2,2,1)
% plot_SimN2(TH,TH_index,N,NPath,alpha,AlphaPath,ERN,SimERN,THonPath,MonthWeekEN,MonthWeekJP,WeekNumber,Tdata,linecolor,20,fs,fn,ft,l);
% subplot(2,2,2)
% plot_Tradeoff2(AlphaM,DM,waves,TH,TH_index,2,linecolor,fs,fn)
% subplot(2,2,3)
% plot_ICU_GOV(TH,TH_index,ICU,SimICU,BED(Tdata),...
%     MonthWeekEN,MonthWeekJP,WeekNumber,Tdata,...
%     linecolor,fs,fn,ft,l);
...
% for i = 1:nTH
%     lineName{i} = ['I, ワクチン接種本数/週 = ' sprintf(ft,TH(i))];
%     lineName2{i} = ['N, ワクチン接種本数/週 = ' sprintf(ft,TH(i))];
% end
...
% lineName2 = cell(nTH,1);
% for i = 1:nTH
%     lineName{i} = ['S, ワクチン接種本数/週 = ' sprintf(ft,TH(i))];
%     lineName2{i} = ['R, ワクチン接種本数/週 = ' sprintf(ft,TH(i))];
% end
...
% lineName = cell(nTH,1);
% for i = 1:nTH
%     lineName{i} = ['\beta, ワクチン接種本数/週 = ' sprintf(ft,TH(i))];
% end
...
% lineName2 = cell(nTH,1);
% for i = 1:nTH
%     lineName{i} = ['\alpha 変異株割合, ワクチン接種本数/週 = ' sprintf(ft,TH(i))];
%     lineName2{i} = ['\delta 変異株割合, ワクチン接種本数/週 = ' sprintf(ft,TH(i))];
% end
...
% lineName2 = cell(nTH,1);
% for i = 1:nTH
%     lineName{i} = ['死亡率, ワクチン接種本数/週 = ' sprintf(ft,TH(i))];
%     lineName2{i} = ['死亡率, ワクチン接種本数/週 = ' sprintf(ft,TH(i))];
% end
...
% % iTH = 2;
% %
% % plot_vaccinepath_jp(paces_ori,sw_vacpath,gradual_paces,V1_medical_ori/ps,V2_medical_ori/ps,medical_jp,V1_elderly_ori/ps,V2_elderly_ori/ps,elderly_jp,ordinary_jp,accept_share,E1,E2,D1,D2,lag,medical_duration,vaccine_figure_loop,SimPeriod,MonthWeekJP,MonthWeekEN,WeekNumber,Tdata,fs,ldfs,fn,language,iTH,TH,ft)
