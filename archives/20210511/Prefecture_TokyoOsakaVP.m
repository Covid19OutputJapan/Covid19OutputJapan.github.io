...
th_off_vector2 = [600,100,80,60,800,50,60]; %2回目の緊急事態宣言の解除基準
th_off_vector3 = [600,100,80,60,800,50,60]; %3回目の緊急事態宣言の解除基準
% 経済回復速度
...
betaT_temp_ini_vec = [0.6,0,0,0,0.2,0,0,0];%0.15
...
%     %------------------------ Updated ----------------------------
    ...
    TH = [3600000,6500000];
    TH_index = TH; 
    %gradual_paces_vector = [10,7];
    gradual_paces_vector = [10,6];
    sw_vacpath_vec = [0,0];
    ...
    if pindex == 1
        ...
        %betaT_temp_ini1 = 0.5;
        %betaT_temp_ini2 = 0.15;
    elseif pindex == 5
        alpha_on = 1.25*alpha_May;
%         TH_index = [300,600,1000];
%         TH = 100:100:1000;
    end 
...
%             title(title_vec(iTH), 'FontSize',fs,'FontName',fn);
            subplot(1,2,2)
...
%                 (InitialValues,alpha_on,alpha_off,th_on1,th_on2,th_off1,th_off2,th_off3,beta_avg*ones(SimPeriod,1),gammaT,deltaT,delta_ss,V,h,k,POP0,hconstant,DRi);
    ...
    for l = 1:2 %1:2 when english version needed
        ...
        set(gcf,'Position',[100,100,1200,500])
        subplot(1,2,1)
        [BackDataN,BackDataAlpha,BackDataERN] ...
