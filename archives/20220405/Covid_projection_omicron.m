function [CumD,GDPLoss,alphaPath,SimData,SimN,SimERN,SimICU_nation,SimICU_pref,SimHospital,SimNewICU_pref,betaPath,beta_tilde_path,SimBRN] = ...
    Covid_projection_omicron(InitialValues,alpha_on,alpha_off,th_on,th_on_N,...
    th_off,th_off_N,betaT,gamma,deltaT,SimICU_nation_rate, SimICU_pref_rate, SimHospital_rate,SimNewICU_pref_rate,h,k,POP0,...
...
for t = 1:T    
    ...
    if SimN(t) >= th_on_N %(SimNewICU_pref(t+1) >= th_on && t >=2) &&  (SimN(t) >= th_on_N)  %
        state = 1;
