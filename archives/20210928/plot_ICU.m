function BackDataICU = ...
    plot_ICU(TH,TH_index,ICU,SimICU,ICU_limit,MonthWeekEN,MonthWeekJP,WeekNumber,Tdata,xline_ind,linecolor,fs,fn,ft,l,Scenario, LineStyles)
% LineWidthVec = [0.5,1.5,0.5,0.5,1.5,0.5];
% LineWidthVec = [0.5, 1.5, 0.5, 1.5, 0.5];
LineWidthVec = [1.5, 1.5, 1.5, 1];
...
for i = 1:length(TH)
    if abs(TH(i) - TH_index(th_wave)) < 0.0001
            ...
            if i == 4
                p(i) = plot([ICU(2:end);SimICU(2:end,i)],'Color',[0.75, 0, 0.75],'LineWidth',LineWidthVec(i), ...
                'LineStyle','-');
            else
