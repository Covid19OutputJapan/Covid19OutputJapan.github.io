function [CumD, GDPLoss, alphapath, SimData, SimN, SimERN, SimICU_nation, SimICU_pref, SimHospital, betaAR1] = ...
        Covid_projection_date_uncertainty(SimData_baseline, alpha_on, alpha_off, ...
    ...
    beta_sim = betaT;
    InitialValues = SimData_baseline(1, :);
    T = length(beta_sim);
    SimData = zeros(T + 1, length(SimData_baseline(1, :)));
    ...
    for t = 1:T
        ...
        % change the dynamics of S and I based on no standard deviation case
        if hconstant == 0
        ...
        SimData(t+1,1) = SimData_baseline(t,1) - SimN(t) - V(t);
        SimData(t+1,2) = SimData_baseline(t,2) + SimN(t) - gammaT(t)*SimData_baseline(t,2) - deltaT(t)*SimData_baseline(t,2);
        SimData(t + 1, 3) = SimData(t, 3) + gammaT(t) * SimData_baseline(t, 2) + V(t); %SimData_baseline? SimData? (check Population is stable)
        SimData(t + 1, 4) = SimData(t, 4) + deltaT(t) * SimData_baseline(t, 2); %Not SimData_baseline
        
        % change the number of newly infected people based on on no standard deviation case
        if hconstant == 0
            SimN(t) = ((1 + h * alpha_sim)^k) * beta_sim(t) * SimData_baseline(t, 1) * SimData_baseline(t, 2) * (1 / POP0);
        elseif hconstant == 1
            SimN(t) = ((1 + (h(2) / h(1)) * alpha_sim)^k) * beta_sim(t) * SimData_baseline(t, 1) * SimData_baseline(t, 2) * (1 / POP0);
        end
        SimData(t + 1, 1) = SimData(t, 1) - SimN(t) - V(t);
        SimData(t + 1, 2) = SimData(t, 2) + SimN(t) - gammaT(t) * SimData(t, 2) - deltaT(t) * SimData(t, 2);
        SimData(t + 1, 3) = SimData(t, 3) + gammaT(t) * SimData(t, 2) + V(t); %SimData_baseline? SimData? (check Population is stable)
        SimData(t + 1, 4) = SimData(t, 4) + deltaT(t) * SimData(t, 2); %Not SimData_baseline
        
        SimICU_nation(t + 1) = max(0, SimICU_nation(t) + ICU_nation_inflow_avg * delta_ICU_nation(t) * SimData(t, 2) ...
            - gamma_ICU_nation * SimICU_nation(t) - deltaT(t) * SimData(t, 2)); %Not SimData_baseline
        SimICU_pref(t + 1) = max(0, SimICU_pref(t) ...
            + ICU_pref_inflow_avg * delta_ICU_pref(t) * SimData(t, 2) ...
            - gamma_ICU_pref * SimICU_pref(t) - deltaT(t) * SimData(t, 2)); %Not SimData_baseline
        SimHospital(t + 1) = max(0, SimHospital(t) + Hospital_inflow_avg * delta_Hospital(t) * SimData(t, 2) ...
            - gamma_Hospital * SimHospital(t)); %Not SimData_baseline
