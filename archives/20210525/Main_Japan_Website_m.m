...
home = '/Users/koheimachi/Dropbox/fujii_nakata/Website/Covid19OutputJapan.github.io/archives/20210525/';
% if iPC == 1
...
if iPC==1
  ...
  NextD = load('/Users/koheimachi/Dropbox/fujii_nakata/Website/Codes/Japan20210601.mat');
...
%     alphaT = flip(0:0.01*TargetAlpha(i)*2/(SimPeriod-1):(0.01*TargetAlpha(i)*2))';
    ...
    [CumD(i),AverageAlpha(i),SimData,SimN, SimICU]= Covid_projection2(InitialValues,alphaT,betaT,gammaT,deltaT,V,h,k,POP0,hconstant,ICU_inflow_avg,gamma_ICU);
...
for i = 1:length(AlphaVals)
    ...
    [CD,AA,SimData,SimN,SimICU] = Covid_projection2(InitialValues,alphaT,betaT,gammaT,deltaT,V,h,k,POP0,hconstant,ICU_inflow_avg,gamma_ICU);
    ...
    for i = 1:length(TargetAlpha)
        ...
        parfor j = 1:Nsim
            ...
            %[V,deltaS,VT] = vaccine_distribution_medical(vacpath,medical,V1_medical,V2_medical,elderly,V1_elderly,V2_elderly, ordinary,elderly_total,delta_averageS,E1,E2,D1,D2,ps,POP0,3,10);
             [V,deltaS,VT] = vaccine_distribution_medical(vacpath,medical,V1_medical,V2_medical,elderly,V1_elderly,V2_elderly, ordinary,elderly_total,delta_averageS,E1,E2,D1,D2,ps,POP0,lag,medical_duration);
%             
%             delta_ssS = delta_averageS*(0.09/1.27);
%             deltaS = delta_averageS*ones(SimPeriod,1);
%             deltaS(VacStart+3:VacStart+3+9+13) = delta_averageS:(delta_ssS-delta_averageS)/22:delta_ssS;
...
%             
            ...
            HS=[HS1,HS2];
            [UB2(j,i)]= Covid_projection2(InitialValues,alphaT,betaS,gammaT,deltaS,V,HS,k,POP0,hconstant,ICU_inflow_avg,gamma_ICU)
        end
    end
