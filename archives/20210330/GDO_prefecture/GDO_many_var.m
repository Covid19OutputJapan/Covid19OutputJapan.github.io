...
%VarVector1 = {'Fujii','Cred','Credserv'};
...
for pindex = 5:5%:length(PrefVector)
    ...
    %figure(2)
    yyaxis left
    plot(Cred,'k','LineWidth',1.5)
    ylabel('Var Index')
    hold on 
    yyaxis right
    plot(GDO_pred_simple,'r-.','LineWidth',1.5)
    hold on
    plot(Fujii_raw,'r-.','LineWidth',1.5)
    hold on
    ...
    legend('Malt(L axis)','Forecasted GDO simple (R axis)', 'Data GDO (R axis)','Data Fujii (R axis)','FontSize',13,'Location','southeast');
    ...
    saveas(gcf,strcat(pref,'-raw'),'fig');
