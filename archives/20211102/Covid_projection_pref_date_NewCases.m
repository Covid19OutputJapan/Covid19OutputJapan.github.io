...
for i = 1:T
    ...
    beta(i) = betaT_Path(i) * seasonality(i);
    ...
    if i == SOE_end
            betaT_ref(i + 1:i + alpha_duration + 1) = beta(i + 1:i + alpha_duration + 1) + beta_additive_path;
            betaT_ref(i + alpha_duration + 2:end) = beta(i + alpha_duration + 2:end) + beta_additive_path(end);
            beta_goal_ref = betaT_ref(i + alpha_duration + 2) - betaT_ref(i);
%             beta_goal_ref = betaT_ref(i + alpha_duration) - betaT_ref(1);
            betaT_ref_test = betaT_ref(i) + beta_goal_ref * beta_jump;
%             betaT_ref_test = betaT_ref(1) + beta_goal_ref * beta_jump;
            beta_jump = (betaT_ref_test - beta(i));
            betaT_Path(i + 1:i + alpha_duration + 1) = beta(i + 1:i + alpha_duration + 1) + beta_additive_path;
            betaT_Path(i + alpha_duration + 2:end) = beta(i + alpha_duration + 2:end) + beta_additive_path(end);
    end
    ...
    if (SimN(i) >= th_on) && i >= SOE_end
        ...
        betaT_Path(i:end) = scale_beta * simple_beta_avg;%betaT_woAR(i:end);
