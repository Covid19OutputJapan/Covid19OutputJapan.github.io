...
if vac_data_save == 1
    ...
    firstcolumns = [round(B(1,1:nM,1)); squeeze(round(B(endmonth-Tdata,1:nM,1)))];
    secondcolumns = [round(B(1,1:nM,2)); squeeze(round(B(endmonth-Tdata,1:nM,2)))];
    thirdcolumns = [round(B(1,1:nM,3)); squeeze(round(B(endmonth-Tdata,1:nM,3)))];
    forthcolumns = [round(B(1,1:nM,4)); squeeze(round(B(endmonth-Tdata,1:nM,4)))];
    for ti = 1:nTH
...
%         titleN(1,7+nTH*(ti-1)) = string(['一般2本目（',sprintf('%.0f',TH(ti)),'）']);
    ...
    writetable(TN,[home 'Figures/' char(pref) '/VaccineData_'  char(pref)  '.xls'],'Sheet','ワクチン見通し','WriteVariableNames',false);
end
%%
