...
ICU_nation(end) = 872; % 2021/08/09 %826; % 2021/08/02 % 678; % 2021/07/26
...
if Scenario_switch == 0
    Scenario = ["基本(Optimistic)", "基本", "基本(Pessimistic)"];
    ScenarioEN = ["Baseline (-1.5 std)", "Baseline", "Baseline(1.5 std)"];
    beta_scale_vec = ones(1, 3) * 0.87; %ones(1,3) * 0.875;
...
elseif Scenario_switch == 2
    Scenario = ["ロックダウン1(-1.5 std)", "ロックダウン1", "ロックダウン1(1.5 std)"];
    ScenarioEN = ["Alternative (-1.5 std)", "Alternative", "Alternative(1.5 std)"];
    lockdown_th = 0.3;
    ...
    figname_main = 'Baseline_MainResults_Oct';
    figname_beta_tilde = 'Baseline Beta Tilde Path';
    figname_beta = 'Baseline Beta Path';
    figname_var = 'Baseline Variant_Share';
    figname_delta = 'Baseline Death Rate transition';
    ...
    figname_dD = 'Baseline dD transition';
elseif Scenario_switch == 3
    Scenario = ["ロックダウン2(-1.5 std)", "ロックダウン2", "ロックダウン2(1.5 std)"];
    ScenarioEN = ["Alternative (-1.5 std)", "Alternative", "Alternative(1.5 std)"];
    lockdown_th = 0.5;

    beta_scale_vec = ones(1, 3) * 0.8;
    figname_main = 'Alternative_MainResults_Oct';
    xline_ind2 = find(date == datetime(2021, 10, 28)); % consider the case where state of emergency is extended
...
for iTH = 1:length(TH)
    ...
    elseif Scenario_switch == 1 %Optimistic
        betaT(1) = betaT(1) * 0.9;
        betaT(2) = betaT(2) * 0.9322; %1.7582;
        betaT(3:th_off_date - 1) = beta_scale * betaT(3:th_off_date - 1);
    elseif Scenario_switch == 2 %Baseline 10月末解除
        betaT(1) = betaT(1) * 0.9;
        betaT(2) = betaT(2) * 1.0813; %1.7582;
        betaT(3) = betaT(3) * 1.0486;
        betaT(4:xline_ind - Tdata) = beta_scale * betaT(4:xline_ind - Tdata);
        betaT(xline_ind - Tdata + 1:xline_ind2 - Tdata) = 2.2;
    elseif Scenario_switch == 3 %Optimistic 10月末解除
        betaT(1) = betaT(1) * 0.9;
        betaT(2) = betaT(2) * 0.9322; %1.7582;
...
for l = 1:2 %1:2 when english version needed
    ...
    if Scenario_switch == 2
        ...
        text(xmin, ICU_limit_pref_vec(xmin) * 0.3, '30%', 'FontSize', fs)
    elseif Scenario_switch == 3
    ...
    xlim([xmin, xmax])
    % ylim([0 650]) %ylim([0 ICU_limit * 1.1])
    ...
    %     text(72,Hospital_limit_vec(72)*0.9 * 3,'300%', 'FontSize', fs)
    % ylim([0 8000])
    %     ylim([0 10000])
    ...
    plot_vaccinepath_separate_percentage(1, VT, V1_medical, V2_medical, V1_elderly, V2_elderly, V1_others, V2_others, ps, MonthWeek(:, l), WeekNumber, Tdata, fs, 8, fn, x_left, x_right, l, POP0);
    xlim([x_left, xmax])
