function plot_beta_India(var_initial,var_initial2,var_share,var_share2,beta,beta_avg,betaT,betaT_Eng,betaT_woAR1,dateD,SimDate,Tdata,MonthWeek,MonthNumber,WeekNumber,fn)
% plot beta
...
plot_var_share2(1,1) = var_initial2;
plot_var_share2(2:length(SimDate)+1,1) = var_share2(:,1);

plot_betaEng(1:tt,1) = beta(end-tt+1:end);
plot_betaEng(tt+1:length(SimDate)+tt,1) = betaT_Eng(:);
...
subplot(1,2,1)
plot(xaxis_vec,plot_var_share(:,1)*100,'-b','LineWidth',2)
hold on 
plot(xaxis_vec,plot_var_share2(:,1)*100,'-r','LineWidth',2)
hold off
ax = gca;
...
legend('イギリス株','インド株','FontSize',10,'FontName',fn)
...
hold on 
plot(xaxis_vec2,plot_betaEng,'-g','LineWidth',2)
hold on 
...
xtickangle(45)
legend('感染率(インド株) W/ AR1','感染率(変異株成長ケース) W/O AR1','感染率(イギリス株のみ) W/ AR1','過去の平均','FontSize',10,'FontName',fn)
