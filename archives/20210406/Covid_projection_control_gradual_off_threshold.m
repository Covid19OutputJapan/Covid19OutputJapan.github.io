function [CumD,GDPLoss,alphapath,SimData,SimN,SimERN] = Covid_projection_control_gradual_off_threshold(InitialValues,alpha_on,alpha_off,th_on1,th_on2,th_off,beta,gamma,delta,V,h,k,POP0,hconstant,alpha_duration,current_alpha)
...
alpha_off_path = current_alpha:(alpha_off-current_alpha)/(alpha_duration+1):alpha_off;
counter = 1;
wave = 0;
wave_off = 0;
for i = 1:T
    ...
    if wave == 0
        ...
        elseif th_on1 > SimN(i) && th_off < SimN(i)
            if state == 1
                alpha = alpha_on;
            elseif state == 0
                if counter <= alpha_duration + 1
    ...
    elseif wave > 0
        ...
        elseif th_on2 > SimN(i) && th_off < SimN(i)
            if state == 1
                alpha = alpha_on;
            elseif state == 0
                if counter <= alpha_duration + 1
