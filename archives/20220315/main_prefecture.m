...
elseif ismac == 1
    HOME        = '/Users/shotaro/GitHub/covid-output/github_Codes/'; %include \ or / at the end
    DATA_PATH   = '/Users/shotaro/Dropbox/fujii_nakata/Website/Codes/Data/';
    FIGURE_PATH = '/Users/shotaro/Dropbox/fujii_nakata/Website/Codes/Figures/';
    FONT_NAME          = 'YuGothic';
end
cd(HOME);
...
% V1_others(Tdata)=V1_others(Tdata-1);
% V2_others(Tdata)=V2_others(Tdata-1);
...
T1_ind = find(date == datetime(2022, 2, 24));
T2_ind = find(date == datetime(2022, 3, 31));
share_T1 = 0.022;
share_T2 = 0.74;
...
SoE_off_date        = find(SimDateEN == datetime(2022, 3, 24)) + 1 + manbo_extension_switch * 3; % まん防 終了...
% 現実世界とのLagを考慮して+1
...
delta_ICU_pref_temp_ini_vec = repmat([-0.3, -0.3], 1, 2); %[0,0,2.0];

%iY
delta_temp_ini_vec = [0, 0, 0];
delta_rho_vec = [0.9, 0.9, 0.85];

delta_ICU_nation_temp_ini_vec =repmat([-0.8, -0.8], 1, 2); %[0.9,0.9,3.0];

delta_newICU_pref_temp_ini_vec = repmat([-0.5, -0.5], 1, 2); %[0,0,2.0];
delta_Hospital_temp_ini_vec = repmat([0, 0], 1, 2); %[2.5,2.5,8.0];


target_relative_severity = repmat([0.2, 0.15], 1, 2);
manbo_beta_shock_vec = [0, 0, 0.2, 0.2] ;
manbo_legend         = ["無し","無し","有り","有り"];
manbo_legend_EN      = ["Without AE","Without AE","With AE","With AE"];
manbo_beta_rho_vec = repmat([0.99, 0.99], 1, 2);
severity_new_pref_standard_vector = target_relative_severity / omicron_realtive_severity;
severity_pref_standard_vector     = target_relative_severity / omicron_realtive_severity;
hospitalization_standard_vector   = target_relative_severity / omicron_realtive_severity;
severity_nation_standard_vector   = target_relative_severity / omicron_realtive_severity;
...
for iY = 1:nY
    lineNameJP{iY}          = ['第5波の重症化率の',num2str(yvec(iY)*100),'%', '　AE', char(manbo_legend(iY))];
    lineNameEN{iY}          = ['Severity Rate ',num2str(yvec(iY)*100),'% (Relatieve to the 5th Wave) ', char(manbo_legend_EN(iY))];
end
...
for iX = 1:nX %different figures
    ...
    for iY = 1:nY %different curves within a figure
        ...
        manbo_beta_shock = manbo_beta_shock_vec(iY);
        manbo_beta_rho   = manbo_beta_rho_vec(iY);
        ...
        for iZ = 1:nZ %different curves within a figure
            ...
            betaT(SoE_off_date:end) =betaT(SoE_off_date:end) .* construct_onetime_AR1(manbo_beta_rho, manbo_beta_shock, SimPeriod-SoE_off_date+1);
            betaBox     = [beta; betaT];
            ...
            alphaT          = [ones(SoE_off_date-1,1)*alpha(Tdata); alphaAfterSOE';alpha_off * ones(SimPeriod-SoE_off_date+1,1)];
            ...
            SimICU_pref_rate(1:end)  = SimICU_pref_rate(1:end) *  severity_pref_standard;
            ...
            [DMat(iX,iY,iZ), AlphaMat(iX,iY,iZ), AlphaPath(:, iX,iY,iZ), ...
                ...
                Covid_projection_omicron_date(InitialValues,alpha_on,alpha_off,SoE_date,SoE_off_date,...
                ...
                relative_beta_SOE, beta_jump, beta_goal, seasonality, alphaBox(Tdata + 1:end), state, simple_beta_avg, beta);
...
%     end
    for iX = 1:nX
        titleN = strings(1, 11);
        titleN(1) = "週";
        titleN(1, 2) = append("新規感染者数（", lineNameJP{2}, "）");
        titleN(1, 3) = append("新規感染者数（", lineNameJP{4}, "）");
        titleN(1, 4) = "経済活動";
        titleN(1, 5) = append("実効再生産数（", lineNameJP{2}, "）");
        titleN(1, 6) = append("実効再生産数（", lineNameJP{4}, "）");
        titleN(1, 7) = append("基本再生産数（", lineNameJP{2}, "）");
        titleN(1, 8) = append("基本再生産数（", lineNameJP{4}, "）");
        for i = 1:nY
            titleN(1, 8+i)  = append("入院患者数（", lineNameJP{i}, "）");
            titleN(1, 12+i) = append("重症者数_旧都基準（", lineNameJP{i}, "）");
            titleN(1, 16+i) = append("重症者数_新都基準（", lineNameJP{i}, "）");
        end
        titleN(1, 21)= append("新規死亡者数（", lineNameJP{2}, "）");
        titleN(1, 22)= append("新規死亡者数（", lineNameJP{4}, "）");

        
        TN = table([
            titleN;
            YearMonthWeekJP(Tdata - 7:end - 1), ...
            squeeze(round(BackDataN(:, iX, [2,4], 1) / 7)), ...
            squeeze(round(100 * (1 - BackDataAlpha(:, iX, 2, 1)), 1)), ...
            squeeze(round(BackDataERN(:, iX, [2,4], 1), 2)), ...
            squeeze(round(BackDataBRN(:, iX, [2,4], 1), 2)), ...
            squeeze(round(BackDataHospital(:, iX, :, 1))), ...
            squeeze(round(BackDataICU_pref(:, iX, :, 1))), ...
            squeeze(round(BackDataNewICU_pref(:, iX, :, 1))), ...
            squeeze(round(BackDatadD(:, iX, [2,4], 1) / 7))
            ]);
        writetable(TN, [FIGURE_PATH, char(PREF) '/' char(figfolder) '/BackData_' char(figname_main) '_' char(ScenarioEN(iX)) '.xls'], 'Sheet', '新規感染者数（1日平均）', 'WriteVariableNames', false);
    end
end
