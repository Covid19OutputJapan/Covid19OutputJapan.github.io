function [CumD,GDPLoss,alphapath,SimData,SimN,SimERN,THonPath,SimICU_nation,SimICU_pref,SimHospital,beta] = ...
    Covid_projection_pref_date_NewCases(InitialValues,alpha_on,alpha_off,th_on1,th_on2,...
...
for i = 1:T
    ...
    if (SimICU_nation(i) >= lockdown_th_on *BED_vec(i) || SimICU_pref(i) >= lockdown_th_on * ICU_limit_pref_vec(i)) && i >= SOE_end
        lockdown_counter = 1;
        lockdown_state = 1;
        alpha = alpha_lockdown;
        alphapath(i) = alpha;
...
%         alphapath(i) = alpha;
    ...
    elseif (SimICU_nation(i) < lockdown_th_off * BED_vec(i) && SimICU_pref(i) < lockdown_th_off * ICU_limit_pref_vec(i)) && i >= SOE_end
        ...
        if lockdown_state == 0 % && i >= 19 %&& beta_counter == 0
            if i + alpha_duration <= 156
                ...
                % %                 betaT(i + alpha_duration + 1:end) = beta(i + alpha_duration + 1:end) + beta_additive_path(end);
                %             end
            elseif end_counter == 0 && i + alpha_duration > 156 % && i >= 19
...
%         lockdown_state = 0;
        ...
        if lockdown_counter <= alpha_duration %&& i >= 19
            alpha = alpha_off_lockdown_path(lockdown_counter);
            alphapath(i) = alpha;
            lockdown_counter = lockdown_counter + 1;
            %             beta(i) = betaT(i);
            
        elseif lockdown_counter > alpha_duration
            alpha = alpha_off;
            alphapath(i) = alpha;
            %             beta(i) = betaT(i);
        end
        
    elseif (((lockdown_th_off * BED_vec(i) <= SimICU_nation(i)) && (SimICU_nation(i) < lockdown_th_on * BED_vec(i))) || ((lockdown_th_off * ICU_limit_pref_vec(i) <= SimICU_pref(i)) && (SimICU_pref(i) < lockdown_th_on * ICU_limit_pref_vec(i)))) && i >= SOE_end
        ...
        %             %             beta(i) = betaT(i);
        %         end
        if lockdown_state == 1 && i >= SOE_end
            alpha = alpha_lockdown;
            alphapath(i) = alpha;
        elseif lockdown_state == 0 && i >= SOE_end
            if lockdown_counter <= alpha_duration
                alpha = alpha_off_lockdown_path(lockdown_counter);
                alphapath(i) = alpha;
                lockdown_counter = lockdown_counter + 1;
                %             beta(i) = betaT(i);
...
%     
    ...
    %     if SimICU_nation(i) >= lockdown_th *BED_vec(i) || SimICU_pref(i) >= lockdown_th * ICU_limit_pref_vec(i)
    %         lockdown_state = 1;
    %         alpha = alpha_lockdown;
    %         alphapath(i) = alpha;
    %         if lockdown_wave == 1 && lockdown_counter > (alpha_duration + 1)
    ...
    %             lockdown_counter = lockdown_counter + 1;
    %         end
    %     elseif SimICU_nation(i) < lockdown_th *BED_vec(i) && SimICU_pref(i) < lockdown_th * ICU_limit_pref_vec(i)
    %         if lockdown_state == 1
    %             lockdown_wave = lockdown_wave + 1;
    %         end
    %         lockdown_state = 0;
