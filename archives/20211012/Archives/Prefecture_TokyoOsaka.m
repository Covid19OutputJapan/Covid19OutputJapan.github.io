...
linecolor = {'blue', 'black', 'blue'}; %{'blue', 'black', 'red'};
language = {'EN','JP'};
%======================================================================%
...
%--- Eliminate the effects of vaccination from delta ---%
ICU_ss = delta_average*(0.3916/1.62); %Share of the death rate among youth to the death rate of all populaiton
share = ((1 - sum(VD_elderly(1:end-2))/elderly_total) * (delta_average - ICU_ss) ...
    + (1-sum(VD_ordinary(1:end-2))/(ordinary_total))*ICU_ss)/delta_average;
ICU_wo_vaccination = delta_average/share; %Eliminate the effects of vaccination in the past average value
...
%     betaT = beta_AR1(betaT_temp_ini, beta_rho, betaT, start_beta);
    ...
    betaT_woAR = betaT;
    betaT = beta_AR1(betaT_temp_ini, beta_rho, betaT, start_beta);
    betaT(1)=betaT(1)*1.41;
    betaT(2)=betaT(2)*1.22;
    betaT(3:5)=beta_scale*betaT(3:5);
...
%     ylim([0 20])
    ...
    plot_Alpha(alpha,AlphaPath,TH,TH_index,MonthWeek(:,l),WeekNumber,MonthNumber,Tdata,{'black','black','black'},ft,fs,fn,l)
