...
if pindex == 1
    ...
    %alpha_scale = 0.94;
    alpha_scale = 1.0;
%     alpha_on = (0.25 * alpha_May + 0.75 * alpha_Jan); %alpha_Jan; %alpha_scale * (0.5 * alpha_May + 0.5 * alpha_Jan);
    alpha_on = (0.5 * alpha_May + 0.5 * alpha_Nov); %alpha_Jan; %alpha_scale * (0.5 * alpha_May + 0.5 * alpha_Jan);
    h_scale = 1;
...
%             saveas(figure(240), [home 'Figures/' char(pref)  '/Death_rate_DR' '_'  sprintf('%.0f', paces_ori),  '.png']);
%         end
...
for l = 1:2 %1:2 when english version needed
    ...
    subplot(2, 2, 1)
    [BackDataN, BackDataAlpha, BackDataERN] = plot_SimN_DR_km(TH, TH_index, N, NPath, alpha, ...
        AlphaPath, ERN, SimERN, THonPath, MonthWeekEN, MonthWeekJP, WeekNumber, Tdata, ...
        linecolor, fs, fn, ft, l,ldfs);
...
xlim([Tdata-31 Tdata+41])
% if figure_save == 1
%     saveas(figure(10), [home 'Figures/' char(pref)  '/Variant_Share.png']);
% end
