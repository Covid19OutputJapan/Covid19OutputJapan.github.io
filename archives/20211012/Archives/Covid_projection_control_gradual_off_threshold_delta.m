function [CumD,GDPLoss,alphapath,SimData,SimN,SimERN] = Covid_projection_control_gradual_off_threshold_delta(InitialValues,alpha_on,alpha_off,th_on1,th_on2,th_on3,th_off1,th_off2,th_off3,beta,gamma,delta,delta_ss,delta_th,V,h,k,POP0,hconstant,alpha_duration,current_alpha)
...
th_on1_ori = th_on1;
th_on2_ori = th_on2;
th1 = find(delta < delta_th,1);
th2 = find(delta < delta_ss,1);
th_on1i = th_on1:(th_on2-th_on1)/(th1):th_on2;
th_on2i = th_on2:(th_on3-th_on2)/(th2):th_on3;
for i = 1:T
    if i < th1
        th_on1 = th_on1i(i);
    elseif i >= th1 && i < th2
    ...
    elseif delta(i) <= delta_th && delta(i) > delta_ss 
