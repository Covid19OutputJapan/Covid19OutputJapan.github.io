...
for pindex = 4:4 %length(PrefVector)
    close all
...
%     betaT is determined in beta_option, so this line should be removed.
    ...
    elseif beta_option == 4
        var_infection = 0.5;  % variant's infection rate (compared to nomral infection rate)
        var_ss = 0.5;         % steady-state share of variant
        ...
        beta_bar = mean(beta_sample)/(1+var_infection*var_ss*var_initial);
    ...
    for i = 1:length(TL)
        if pindex <= 4
            [DMat(1,i),AlphaMat(1,i),AlphaPath(:,i,1),SimData,NPath(:,i,1),SimERN(:,i,1)] = Covid_projection_control_gradual(InitialValues,altA,alpha_off,th_on,TL(i)*7,betaT,gammaT,deltaT,V,h_all,k,POP0,hconstant,4); %1 -> 4 gradual 3/15
        elseif pindex > 4
            [DMat(1,i),AlphaMat(1,i),AlphaPath(:,i,1),SimData,NPath(:,i,1),SimERN(:,i,1)] = Covid_projection_control_gradual_off(InitialValues,altA,alpha_off,th_on,TL(i)*7,betaT,gammaT,deltaT,V,h_all,k,POP0,hconstant,4,alpha(end)); %1 -> 4 gradual 3/15
        end
    end
    %---------------------------------------------------------------%
    ...
    for i = 1:length(DR)
        ...
        elseif pindex > 4
            [DMat(2,i),AlphaMat(2,i),AlphaPath(:,i,2),SimData,NPath(:,i,2),SimERN(:,i,2)] = Covid_projection_control_gradual_off(InitialValues,altA,alpha_off,th_on,TL_index(1)*7,betaT,gammaT,deltaT,V,h_all,k,POP0,hconstant,1+DR(i),alpha(end));
        end
    end
    %-----------------------------------------------------%
    %
