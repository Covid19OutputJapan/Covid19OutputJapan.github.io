...
TH = [7000000,8400000];
TH_index = TH;
gradual_paces_vector = [1,3];
sw_vacpath_vec = [0,0];
ind_date_vec = [find(date == datetime(2021,7,15)),find(date == datetime(2021,7,8))]; %Date by which the first shots should finish

VT3share_vec = [0.5,0.55];
lag_vec = [3,3];
medical_duration_vec = [1,1];
...
%     var_initial2 = 0.13
    ...
    h_scale = 1;
    beta_shock_after_emergency = 0.1;
...
%         ylim([0 ps*paces_ori*1.2/10000])
    ...
    if figure_save == 1
        saveas(figure(200+iTH), [home 'Figures/' char(pref) '/Vaccine_path_' num2str(paces_ori) '.png']);
    end
