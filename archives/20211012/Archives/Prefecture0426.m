...
SimCases = 2; % 1 = Different thresholds (OFF); 2 = Different durations for economic recovery ; 3 = different vaccine policy; 4 = different infection rate % 5 = Different thresholds (ON)
...
th_on_vector = [750,700,400,350,1500,350,350]; % 1回目の緊急時代宣言をする基準
th_on_vector2 = [1250,700,400,350,1500,350,350]; % 2回目の緊急時代宣言をする基準
...
ERN_on_scenario1 = [0.8,0.8,0.8,0.8,0.8,0.8,0.8]; %beta_option = 41
ERN_on_scenario2 = [0.55,0.5,0.5,0.5,0.65,0.5,0.5]; %beta_option = 42
...
var_initial_vector = [0.2531, 0.2909, 0.1335, 0.0909, 0.7721, 0, 0, 0,0]; % 4/23
for pindex = 1:5
    ...
    var_initial_vector(pindex) = var_share(1); %var_share(2)
end
%=====================================================%
disp(var_initial_vector) %[0.3516 0.3963 0.1978 0.1379 0.8443   0 0 0 0]; % 4/23
...
for pindex = 1:1  %:length(PrefVector)
    ...
    for bb = 1:length(beta_option_index)%[beta_option_index]
        ...
        for th_off_index = 1:1 %1:length(th_off_vec)
            ...
            if y == 1
                for i = 1:length(ON)
                    [DMat(1,i),AlphaMat(1,i),AlphaPath(:,i,1),SimData(:,:,i,1),NPath(:,i,1),SimERN(:,i,1)] = Covid_projection_control_gradual_off_threshold_new(InitialValues,altA_on,alpha_off,ON(i),ON(i),th_off,th_off2,betaT,gammaT,deltaT,V,h,k,POP0,hconstant,1+DR_index(1),alpha(end));
                end
                ON = ON/7;
            ...
            elseif y == 2
                for i = 1:length(DR)
                    [DMat(2,i),AlphaMat(2,i),AlphaPath(:,i,2),SimData(:,:,i,2),NPath(:,i,2),SimERN(:,i,2)] = Covid_projection_control_gradual_off_threshold_new(InitialValues,altA_on,alpha_off,th_on,th_on2,th_off,th_off2,betaT,gammaT,deltaT,V,h,k,POP0,hconstant,1+DR(i),alpha(end));
                end
                
                %---- 3. Different ERN on ---%
            elseif y == 3
                for i = 1:length(AON)
                    [DMat(3,i),AlphaMat(3,i),AlphaPath(:,i,3),SimData(:,:,i,3),NPath(:,i,3),SimERN(:,i,3)] = Covid_projection_control_gradual_off_threshold_new(InitialValues,AON(i),alpha_off,th_on,th_on2,th_off,th_off2,betaT,gammaT,deltaT,V,h,k,POP0,hconstant,1+DR_index(1),alpha(end));
                end
            ...
            elseif y == 4
                for i = 1:length(var_infection_vector)
                    ...
                    betaT = beta_bar*(1+var_infection*var_share);
                    [DMat(4,i),AlphaMat(4,i),AlphaPath(:,i,4),SimData(:,:,i,4),NPath(:,i,4),SimERN(:,i,4)] = Covid_projection_control_gradual_off_threshold_new(InitialValues,altA_on,alpha_off,th_on,th_on2,th_off,th_off2,betaT,gammaT,deltaT,V,h,k,POP0,hconstant,1+DR_index(1),alpha(end));
                end
                %---- 5  Different Thresholds to List  ---%
            elseif y ==5
                for i = 1:length(TL)
                    [DMat(5,i),AlphaMat(5,i),AlphaPath(:,i,5),SimData(:,:,i,5),NPath(:,i,5),SimERN(:,i,5)] = Covid_projection_control_gradual_off_threshold_new(InitialValues,altA_on,alpha_off,th_on,th_on2,TL(i)*7,TL(i)*7,betaT,gammaT,deltaT,V,h,k,POP0,hconstant,1+DR_index(1),alpha(end));
                end
            end
...
%                 end
            ...
            if figure_save == 1
                ...
                elseif beta_option == 42
                    ...
                    elseif y == 5
                        saveas(figure(42016),[home 'Figures/' char(pref) '/Thresholds' sprintf('%.0f', beta_option) '.png']);
                        saveas(figure(42161),[home 'Figures/' char(pref) '/Thresholds' sprintf('%.0f', beta_option) '_jp.png']);
                    end
                else
                    if y == 1
...
if length(beta_option_index) == 3
    ...
    %--- Number of cumulative deaths ---%
    AlphaM1 = AlphaM_beta(:,y,1);
    DM1 = DM_beta(:,y,1);
    waves1 = waves_beta(:,y,1);
    subplot(3,2,2)
    ...
    hold on
    scatter(AlphaM1(abs(TH - TH_index(1)) < 0.0001),DM1(abs(TH - TH_index(1)) < 0.0001),250,'red','filled');
    hold on
    scatter(AlphaM1(abs(TH - TH_index(2)) < 0.0001),DM1(abs(TH - TH_index(2)) < 0.0001),250,'blue','filled');
    if l == 1
        xlabel('Output Loss (hundred million yen)','FontSize',20)
        ylabel('Cumulative Deaths','FontSize',20)
        title('Relationship between Covid-19 and output','FontSize',20,'FontWeight','normal')
    ...
    %--- Number of cumulative deaths ---%
    AlphaM2 = AlphaM_beta(:,y,2);
    DM2 = DM_beta(:,y,2);
    waves2 = waves_beta(:,y,2);
    subplot(3,2,4)
    ...
    hold on
    scatter(AlphaM2(abs(TH - TH_index(1)) < 0.0001),DM2(abs(TH - TH_index(1)) < 0.0001),250,'red','filled');
    hold on
    scatter(AlphaM2(abs(TH - TH_index(2)) < 0.0001),DM2(abs(TH - TH_index(2)) < 0.0001),250,'blue','filled');
    if l == 1
        xlabel('Output Loss (hundred million yen)','FontSize',20)
        ylabel('Cumulative Deaths','FontSize',20)
        title('Relationship between Covid-19 and output','FontSize',20,'FontWeight','normal')
    ...
    %--- Number of cumulative deaths ---%
    AlphaM3 = AlphaM_beta(:,y,3);
    DM3 = DM_beta(:,y,3);
    waves3 = waves_beta(:,y,3);
    subplot(3,2,6)
    ...
    hold on
    scatter(AlphaM3(abs(TH - TH_index(1)) < 0.0001),DM3(abs(TH - TH_index(1)) < 0.0001),250,'red','filled');
    hold on
    scatter(AlphaM3(abs(TH - TH_index(2)) < 0.0001),DM3(abs(TH - TH_index(2)) < 0.0001),250,'blue','filled');
    if l == 1
        xlabel('Output Loss (hundred million yen)','FontSize',20)
        ylabel('Cumulative Deaths','FontSize',20)
        title('Relationship between Covid-19 and output','FontSize',20,'FontWeight','normal')
