...
% parameters for ICU
ICU_death = 10; % ratio of ICU/death
gamma_ICU = 7/18; % Recovery rate from ICU
...
% 緊急事態宣言の発令基準
th_on_vector = [800,300,300,300,500,300,300];
...
th_off_vector = [300,100,110,80,825,50,60]; %1回目の緊急事態宣言の解除基準
th_off_vector2 = [300,100,80,60,500,50,60]; %2回目の緊急事態宣言の解除基準
th_off_vector3 = [300,100,80,60,500,50,60]; %3回目の緊急事態宣言の解除基準

% 経済回復速度
%DRi_vector = [8,6,6,6,6,6,6,6];
DRi = 6;
...
betaT_temp_ini_vec = [0.8,0,0,0,0,0,0,0];%0.15
...
TL_vector = {30:10:100,70:10:100,80:10:130,50:10:100,[100:100:1000, 1060],20:10:100,10:10:100,50:10:100}; % 解除基準分析をコントロールしている Cell array
TL_index_vector = {[30,50,100],[100,80],[130,110],[100,80],[250,500,100],[50,20],[60,10]}; %色付けして強調するものを決める
...
for pindex = [1] %:length(PrefVector) %change this parameter for prefecture
    ...
    %         covid = importdata([home 'Covid_weekly_NewV.csv']);  % Import weekly Covid data by prefecture
    %     end
    %     Data_prefVaccine = covid.data(strcmp(covid.textdata(2:end,1),pref),:);
    %
    %
    %     E_total = cum_to_new(Data_prefVaccine(:,8));
    %     E_first = cum_to_new(Data_prefVaccine(:,9));
    %     E_second = cum_to_new(Data_prefVaccine(:,10));
    %     M_total = cum_to_new(Data_prefVaccine(:,11));
    %     M_first = cum_to_new(Data_prefVaccine(:,12));
    %     M_second = cum_to_new(Data_prefVaccine(:,13));
    % %     M_total(find(M_total>0,1)) = 0;
    % %     M_first(find(M_first>0,1)) = 0;
    % %     M_second(find(M_second>0,1)) = 0;
    % %     varname1 = V1_medical;
    % %     varname2 = M_first;
    % %     disp(sum(varname1)-varname2(find(varname2>0,1,'last')));
    %     %------------------------ Updated ----------------------------
    ...
    if pindex == 1
        %alpha_on = alpha_May;
        alpha_on1 = alpha_May;
        alpha_on2 = 0.5*alpha_May+0.5*alpha_Jan;
        alpha_on_vector_sim = [alpha_on1, alpha_on2];
        betaT_temp_ini1 = 0.6; %0.5;
        betaT_temp_ini2 = 0.6; %0.15;
        betaT_temp_ini_vec_sim = [betaT_temp_ini1,betaT_temp_ini2];
        TH_index = [30,50,100]; %[0.3]
        fig_title_vec = ["Baseline","Alternative"];
        data_title_vec = ["Baseline","Alternative"];
    elseif pindex == 5
        %         alpha_on = 1.25*alpha_May;
        %         TH_index = [500,700,1060]; %[0.3]
        %         TH = [100:100:1000,1060];
        alpha_on1 = 1.25*alpha_May;
        TH_index1 = [300,500,800];
        TH1 = [200:100:800];
        
        alpha_on2 = 1*alpha_May;
        TH_index2 = [300,500,800];
        TH2 = [200:100:800];
        
        betaT_temp_ini1 = 0.2; %0.5;
        betaT_temp_ini2 = 0.2; %0.15;
        betaT_temp_ini_vec_sim = [betaT_temp_ini1,betaT_temp_ini2];
        
        alpha_on_vector_sim = [alpha_on1, alpha_on2];
        TH_vec = {TH1,TH2};
        TH_index_vec = {TH_index1,TH_index2};
        fig_title_vec = ["Baseline","Alternative"];
        data_title_vec = ["Baseline","Alternative"];
    end
    ...
    for iAlpha = 1:length(alpha_on_vector_sim)
        ...
        for iTH = 1:length(TH)
            ...
            %alpha_off = mean(alpha((dateEN >= datetime(2020,8,6)) & (datetime(2020,11,26)>= dateEN ))); % output loss without the state of emergency
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Simulation %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [DMat(iTH),AlphaMat(iTH),AlphaPath(:,iTH),SimData(:,:,iTH),NPath(:,iTH),SimERN(:,iTH),ICU(:,iTH)] ...
                = Covid_projection_ICU...
                (InitialValues,alpha_on,alpha_off,th_on,th_off1,th_off2,th_off3,betaT,gammaT,deltaT,V,h,k,POP0,hconstant,DRi,alpha(end),state,ICU_death,gamma_ICU);
