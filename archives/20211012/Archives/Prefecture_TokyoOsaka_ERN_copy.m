...
[delta,beta_tilde,ERN,beta,ICU_inflow_nation, ICU_inflow_pref,...
    gammaT,delta_average,ICU_inflow_avg_nation,ICU_inflow_avg_pref,delta_sample,beta_avg]...
    = Time_Series_Average(S,I,D,ICU_nation,ICU_pref,dD,N,Tdata,SimPeriod,...
...
%     betaT = beta_AR1(betaT_temp_ini, beta_rho, betaT, start_beta);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Simulation %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [DMat(iTH),AlphaMat(iTH),AlphaPath(:,iTH),SimData(:,:,iTH),NPath(:,iTH),SimERN(:,iTH),THonPath(:,iTH),SimICU_nation(:,iTH),SimICU_pref(:,iTH),betaShock] ...
        = Covid_projection_pref...
...
%         Tdata,linecolor,fs,fn,ft,l);
    ...
    if l == 1
        title('ICU (Naitonal Standard)','FontSize',fs,'FontWeight','normal')
        xticklabels(MonthWeekEN(WeekNumber==1))
        %xticklabels(datestr(MonthWeekEN(xtick1), 'mmm-yy'))
    elseif l == 2
    ...
    jTitle = '新規死亡者数（1日平均）';
    plot_function(TH,TH_index,dD/7,round(Sim_dDPath/7,0),...
...
if data_save == 1
    titleN = strings(1,1+length(TH_index)*4);
    titleN(1) = "週";
    for ti = 1:length(TH_index)
        ...
        titleN(1,1+length(TH_index)*4+ti) = string(['重症者数_地方基準（',sprintf(ft,TH_index(ti)),'）']);
    end
...
plot_function(TH,TH_index,delta.*ICU_inflow_nation,ICU_inflow_avg_nation*deltaICUPath,...
    MonthWeekEN,MonthWeekJP,WeekNumber,Tdata,xline_ind,linecolor,lineName,...
    eTitle,jTitle,fs,lgfs,fn,ft,yft,column,l,show_other,'SouthOutside')
plot([NaN(Tdata - 1, 1); ones(SimPeriod + 1, 1) * delta_past_avg*ICU_inflow_avg_nation], '--k', 'LineWidth', 1.5, 'DisplayName', '過去17週間平均')
...
jTitle = '重症化率の推移(地方基準)';
plot_function(TH,TH_index,delta.*ICU_inflow_pref,ICU_inflow_avg_pref*deltaICUPath,...
    MonthWeekEN,MonthWeekJP,WeekNumber,Tdata,xline_ind,linecolor,lineName,...
    eTitle,jTitle,fs,lgfs,fn,ft,yft,column,l,show_other,'SouthOutside')
