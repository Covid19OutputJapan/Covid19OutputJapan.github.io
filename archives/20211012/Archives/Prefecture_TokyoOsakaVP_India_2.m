...
%                 (InitialValues,alpha_on,alpha_off,th_on1,th_on2,th_off1,th_off2,th_off3,betaT,gammaT,deltaT,deltaT(1),V,h,k,POP0,hconstant,DRi,state,ICU_inflow_avg,gamma_ICU,beta_shock_after_emergency,rho_after_emergency,alpha_jump);
...
%         xlim([20 Tdata])
        ...
        if figure_save == 1
            if variant_India == 1
                saveas(f,[home 'Figures/' char(pref) '/Vaccine_India_' char(lng) '.png']);
            else
                saveas(f,[home 'Figures/' char(pref) '/Vaccine_' char(lng) '.png']);
            end
        end
    ...
    if data_save == 1
        ...
        if variant_India == 1
            writetable(TN,[home 'Figures/' char(pref) '/BackData_Vaccine_India_' char(pref)  '.xls'],'Sheet','新規感染者数（1日平均）','WriteVariableNames',false);
            writetable(TAD,[home 'Figures/' char(pref) '/BackData_Vaccine_India_' char(pref) '.xls'],'Sheet','経済損失と死亡者数','WriteVariableNames',false);
        else
