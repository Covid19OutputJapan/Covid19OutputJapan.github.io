...
SimCases = 2; % 1 = Different thresholds(ON); 2 = Different durations for economic recovery ; 3 = different ERN ; 4 = different infection rate % 5 = Different thresholds (OFF)
...
th_off_vector42 = {[550,350],[100,80],[80,110],[60,80],[100,40],[50,20],[60,10]}; %Also change TL_index
        ...
        for bb = 1:length(beta_option_index)%[beta_option_index]
            ...
            for th_off_index = 1:1 %1:length(th_off_vec)
                ...
                elseif y == 2
                    for i = 1:length(DR)
                        if pindex < 5
                            [DMat(2,i),AlphaMat(2,i),AlphaPath(:,i,2),SimData(:,:,i,2),NPath(:,i,2),SimERN(:,i,2)] = Covid_projection_control_gradual_off_threshold_delta2(InitialValues,altA_on,alpha_off,th_on,th_on2,th_off,th_off2,th_off3,betaT,gammaT,deltaT,delta_ss,V,h,k,POP0,hconstant,1+DR(i),alpha(end));
                        else
                            [DMat(2,i),AlphaMat(2,i),AlphaPath(:,i,2),SimData(:,:,i,2),NPath(:,i,2),SimERN(:,i,2)] = Covid_projection_control_gradual_threshold_delta2(InitialValues,altA_on,alpha_off,th_on,th_on2,th_off,th_off2,th_off3,betaT,gammaT,deltaT,delta_ss,V,h,k,POP0,hconstant,1+DR(i));
                        end
                    end
...
disp('Check Alpha path to see whether the economy recovers gradually.')