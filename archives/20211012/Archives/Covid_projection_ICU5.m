function [CumD,GDPLoss,alphapath,SimData,SimN,SimERN,THonPath,SimICU,beta] = ...
    Covid_projection_ICU5(InitialValues,alpha_on,alpha_off,th_on1,th_on2,...
...
%     end
    ...
    SimICU(i+1) = max(0, SimICU(i) + new_ICU*delta_ICU(i)*SimData(i,2) - gamma_ICU*SimICU(i) - delta(i)*SimData(i,2));
    if th_on <= SimN(i)
        alpha = alpha_on;
        beta_shock = beta_shock_after_emergency;
        counter = 1;
