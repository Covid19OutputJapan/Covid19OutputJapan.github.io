...
for i = 1:length(TH)
    if abs(TH(i) - TH_index(th_wave)) < 0.0001
        if i == 4
            plot([pastPath1;SimPath1(:,i)],'Color',[0.75, 0, 0.75],'LineWidth',lineWidth(i),'DisplayName',lineName{i})
        else
            plot([pastPath1;SimPath1(:,i)],linecolor{th_wave},'LineWidth',lineWidth(i),'DisplayName',lineName{i})
        end
        hold on
        th_wave = th_wave + 1;
...
%     xticklabels(MonthWeekEN(MonthNumber==[1,4,7,10]))
