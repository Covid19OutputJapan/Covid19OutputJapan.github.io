...
alpha_soe_path(1:17) = alpha_soe_path_ori(2:18);
...
% beta_shock_after_emergency = 0.3;
% beta_shock = beta_shock_after_emergency;
...
for i = 1:T
    ...
    if (SimN(i) >= th_on) && i >= SOE_end
        ...
        betaT(i + 1) = betaT(i + 1) - beta_additive_path(1);
...
%         if i < T && i >= SOE_end  && lockdown_wave >= 1
%             betaT(i+1) = betaT(i+1)*(1+beta_shock);
%         end
%         %beta(i) = beta(i)*(1+beta_shock);
%         beta_shock = beta_shock * rho_after_emergency;
        ...
        if lockdown_state == 0 % && i >= 19 %&& beta_counter == 0
            ...
            if i + alpha_duration <= 156 && lockdown_wave >= 1
                
                %                 beta(i:i + alpha_duration) = betaT(i);
                ...
                % %                 betaT(i + alpha_duration + 1:end) = beta(i + alpha_duration + 1:end) + beta_additive_path(end);
                %             end
            elseif end_counter == 0 && i + alpha_duration > 156 && lockdown_wave == 1
