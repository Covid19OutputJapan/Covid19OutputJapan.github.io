...
Prefs_Vars = readtable('pref_vars_2.csv'); %使用する変数を読み込む


k_set =1; %1:RDEIから予測 2:RDEI予測しない
Kasai = 1; %1:KasaGDPをメインで使う
simple = 1; %1:単純平均 0:重みづけ平均 2:重回帰
...
for pindex = 1:47%:length(PrefVector)    sagaは34
    ...
    if pindex == 1 |pindex == 4 | pindex == 19 |pindex == 33 |pindex == 35 |pindex == 41  %saga 34
        ...
        plot(FujiiKasaigdp,'b-','LineWidth',1.5) %４つ目
        hold on
        plot(Kasaiiip,'k-','LineWidth',1.5)
        
        yyaxis left
        hold on
        plot(TDBall,'r-.','LineWidth',1.5) %４つ目'b-.',
        hold on
        plot(TDBseizou,'r-.','LineWidth',1.5) %４つ目'b-.',
    
        hold on
        plot(TDBoroshi,'k-.','LineWidth',1.5) %４つ目
        hold on
        plot(TDBkouri,'y-.','LineWidth',1.5) %４つ目
        hold on
        plot(TDBunyu,'c-.','LineWidth',1.5) %４つ目
        legend('FKgdp','Kasaiiip','all','seizou','oroshi','kouri','unyu','FontSize',13,'Location','southeast');
