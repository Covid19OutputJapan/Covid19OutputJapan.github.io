...
Prefs_Vars = readtable('pref_vars.csv');
...
for pindex = 1:47%:length(PrefVector)   
    ...
    VarVector = {'Fujii','Malt','Cred','Credserv','Pos','Trav','Event','Rest','Job','Eventinfo','Travelinfo','Movieinfo','Depart','Conveni','Kaden','Drugstore','TDB'};   
    VarVector_new = {'Tdbagri','Tdbkinyuu','Tdbfudosan','Tdbseizou','Tdboroshi','Tdbkouri','Tdbunyu','Tdbsurvice','Tdbother','koujihi','yuirail','airport','hotel','fish','elec','ship','kankou3','kankou2','kankou1','car2','car1','deposit','loan','machine2','machine1','ex2','ex1','wage2','wage1','employ','cpi','di','ci','fusai2','fusai1','tosan2','tosan1','ryoko','setsubi','infra2','infra1','newcar3','newcar2','newcar1'}
    %ここは消す
    %変数変えるときはここをいじる
    VarVector = horzcat(VarVector,VarVector_new)
    ...
    %%共通のデータを変数に格納
    for varindex = 1:size_var %lag以外をimport
        VarVector{varindex}
    ...
    add_size = size(Pref_Vars,1);
 
    
    text1 = "Data_cell = {Fujii,Malt,Cred,Credserv,Pos,Trav,Event,Rest,Job,Eventinfo,Travelinfo,Movieinfo,Depart,Conveni,Kaden,Drugstore,TDB,Tdbagri,Tdbkinyuu,Tdbfudosan,Tdbseizou,Tdboroshi,Tdbkouri,Tdbunyu,Tdbsurvice,Tdbother,koujihi,yuirail,airport,hotel,fish,elec,ship,kankou3,kankou2,kankou1,car2,car1,deposit,loan,machine2,machine1,ex2,ex1,wage2,wage1,employ,cpi,di,ci,fusai2,fusai1,tosan2,tosan1,ryoko,setsubi,infra2,infra1,newcar3,newcar2,newcar1";
    %text1 = "Data_cell = {Fujii,Malt,Cred,Credserv,Pos,Trav,Event,Rest,Job,Eventinfo,Travelinfo,Movieinfo,Depart,Conveni,Kaden,Drugstore,TDB,Tdbagri,Tdbkinyuu,Tdbfudosan,Tdbseizou,Tdboroshi,Tdbkouri,Tdbunyu,Tdbsurvice,Tdbother,koujihi,yuirail,airport,hotel,fish,elec,ship,kankou3,kankou2,kankou1,car2,car1,deposit,loan,machine2,machine1,ex2,ex1,wage2,wage1,employ,cpi,di,ci,fusai2,fusai1,tosan2,tosan1,ryoko,setsubi,infra2,infra1,newcar3,newcar2";
    text2 = "";
    text3 = ",Lag};";
    ...
    for add_var_index=1:add_size
        ...
        col_index = strcmp(columns,Pref_Vars(add_var_index)) * kari';
        eval(strcat(Pref_Vars(add_var_index),"=Data(:,",num2str(col_index),");"))
        eval(strcat(Pref_Vars(add_var_index),"=normalize(",Pref_Vars(add_var_index),");"))
    ...
    VarVector_use_raw = {'Fujii','Malt','Cred','Credserv','Pos','Trav','Event','Rest','Job','Eventinfo','Travelinfo','Movieinfo','Depart','Conveni','Kaden','Drugstore','TDB'};
    VarVector_use_raw = horzcat(VarVector_use_raw,VarVector_new) 
    
    VarVector_use = horzcat(VarVector_use_raw,'Lag');
    ...
    if pindex == 4 | pindex == 19 |pindex == 32 | pindex == 33 |pindex == 35 |pindex == 41
        ...
        yyaxis left
        plot(Malt,'r:','LineWidth',1.5) %２つ目'g-.',
        hold on
        plot(Pos,'g:','LineWidth',1.5) %２つ目'g-.',
        hold on
        plot(Cred,'m:','LineWidth',1.5) %3つ目'k-.',
        hold on
        plot(Credserv,'c:','LineWidth',1.5) %４つ目'b-.',
        ...
        legend('Malt(Laxis),','Pos(L axis)','Cred(L axis)','Credserv(L axis)','Forecasted GDO AR1 (R axis)', 'Data Fujii (R axis)','Data GDO (R axis)','FontSize',13,'Location','southeast');
