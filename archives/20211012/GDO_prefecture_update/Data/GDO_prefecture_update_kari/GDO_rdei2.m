...
for pindex = 1:47%:length(PrefVector)    sagaは34
    ...
    for k = k_set:2
        ...
        Data_vintage = vintage_covid.data(strcmp(covid.textdata(2:end,1),pref),:);
        ...
        if k==1
            clearvars -except vintage_covid plot_other acc_plot acctest vintage_date vintage simple savedata Kasai k_set previous qrdei_pred pindex columns covid Datas_struct figure_save fn fs GDO_preds_star_simple home iPC Prefs_Vars PrefVector Result useful_seq;
        else
    ...
    %if true
        ...
        plot(naive_Fujii_raw,'b-.','LineWidth',1.5) %3つ目
        hold on
        ...
        plot(vintage_data,'g-.','LineWidth',1.5)
        ...
        if plot_other ==1 
            ...
            if k_set ~= 2 %%%基本ここを変えればOK
                ...
                legend('Mobility(L axis)','TDB(L axis)','Forecasted GDO (R axis)', 'Data Fujii (R axis)','Data GDO (R axis)',strcat('vintage-',vintage_date),'FontSize',13,'Location','southeast');
            end
        end
        ...
        if plot_mode == 1
            ...
            if acc_plot ==1
                ...
                annotation('textbox',dim_2,'String',[text_acc;horzcat('mean-error: ',num2str(errors'));horzcat('direc-TF 2011~2103 : ',num2str(direcs))],'Edgecolor','none');
            end
        end
...
if strcmp(pref,'Yamanashi')
    ...
    Data_vintage = vintage_covid.data(strcmp(covid.textdata(2:end,1),"Japan"),:);
    vintage_mobility = normalize(Data_vintage(:,17));
