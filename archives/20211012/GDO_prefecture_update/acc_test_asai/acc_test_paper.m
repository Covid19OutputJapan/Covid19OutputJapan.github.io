...
if iPC==1
    ...
    home = '/Users/asaihiroyuki/Dropbox/fujii_nakata/Website/Codes/GDO_prefecture_update/acc_test_asai/';
    %home_2 = '/Users/asaihiroyuki/Dropbox/fujii_nakata/Website/Codes/GDO_prefecture_update/';
end
...
savedata =1; %1:acctest.matで保存（共有用に必要なので基本1)
...
for pindex = 1:47%:length(PrefVector)    sagaは34
    ...
    for k = k_set %accテスト用の臨時 本来k_set:2
        ...
        for valid = 1:tvalid
            ...
            Pref_Vars = setdiff(Pref_Vars,{'jobiffer','koujihi','hhexp','pos','eventticket'});
            ...
            if k==1
                %clearvars -except testscore simple savedata Kasai k_set previous qrdei_pred pindex columns covid Datas_struct figure_save fn fs GDO_preds_star_simple home iPC Prefs_Vars PrefVector Result useful_seq;
            else
        ...
        errors_pref = abs(GDO_pred_star_test(Tdata-tvalid+1:Tdata)-GDO_raw(Tdata-tvalid+1:Tdata));
        error_mean = mean(errors_pref);
        Datas_struct.(pref).error = error_mean;
        
        errors_pref_ = GDO_pred_star_test(Tdata-tvalid+1:Tdata)-GDO_raw(Tdata-tvalid+1:Tdata);
        error_mean_ = mean(errors_pref_);
        Datas_struct.(pref).error_ = error_mean_;
        
        direc_test = sign(GDO_pred_star_test(Tdata-tvalid+1:Tdata) - GDO_pred_star_test(Tdata-tvalid:Tdata-1));
        direc_true = sign(GDO_raw(Tdata-tvalid+1:Tdata) - GDO_raw(Tdata-tvalid:Tdata-1));
        direc = [];
        for i = 1:tvalid
        ...
        errors_prefs_ = horzcat(errors_prefs_,errors_pref_);
    ...
    %if pindex == 4 | pindex == 19 |pindex == 33 |pindex == 35 |pindex == 41  %saga 34
    ...
    nexttile
        ...
        legend('Estimated GDO','True GDO','FontSize',13,'Location','southeast');
        ylabel('GDO')
        xticks(xtick1)
        ...
        title(strcat(pref),'FontSize',18,'FontWeight','normal')
        ...
        [horzcat('error-avg : ',num2str(Datas_struct.(pref).error)),newline,horzcat('direc : ',num2str(direc))]
...
if strcmp(pref,'Yamanashi')
    ...
    JCER = [100	101.6901699	97.87582587	90.95948383	89.1380334	95.70606363	95.86186857	96.4023196	98.33337641	99.64011438	99.68061472	99.55540006	98.73980912	97.95453603	99.02045085	99.00924349	98.50513257	99.58015316];%	100.1642848]; 7月データは使わない
    ...
    S = load("GDO_preds_star_simple.mat");
    ...
    GDO_pred_star_test = GDO_preds_star_test(:,48);
    GDO_raw = S.GDO_preds_star_simple(1:18,48);
    ...
    plot(GDO_preds_star_test(:,48),'r-.','LineWidth',1.5) %２つ目
    hold on
    plot(S.GDO_preds_star_simple(1:18,48),'b-','LineWidth',1.5) %４つ目
    hold on
    %plot(GDO_preds_star_simple(:,41),'b-.','LineWidth',1.5) %４つ目
    %hold on
    plot(JCER,'k--*','LineWidth',1.5) %４つ目
    hold on
    plot(qe,'k-o','LineWidth',1.5) %４つ目
    ...
    legend('Estimated GDO','True GDO','JCER','QE','FontSize',13,'Location','southeast');
    ...
    %annotation('textbox',dim,'String',[horzcat('error : ',num2str(error_summary)),newline,horzcat('direc : ',num2str(direc_summary))],'Edgecolor','none');
    
    %全国で平均した精度
    %annotation('textbox','String',[horzcat('error : ',num2str(error_summary)),newline,horzcat('direc : ',num2str(direc_summary))],'Edgecolor','none');
    ...
    title('Japan','FontSize',10,'FontWeight','normal')
