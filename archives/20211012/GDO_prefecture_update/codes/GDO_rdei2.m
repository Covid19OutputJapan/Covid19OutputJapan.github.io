...
for pindex = 1:47%:length(PrefVector)    
    ...
    for k = k_set:2
        ...
        if k==1
            clearvars -except today_date vintage_covid plot_other acc_plot acctest vintage_date vintage simple savedata Kasai k_set previous qrdei_pred pindex columns covid Datas_struct figure_save fn fs GDO_preds_star_simple home iPC Prefs_Vars PrefVector Result useful_seq;
        else
    ...
    if pindex == 33 |pindex == 41  %saga 34
    %if pindex == 3
...
if strcmp(pref,'Yamanashi')
    ...
    if acc_plot ==1
        ...
        %annotation('textbox',dim_2,'String',[horzcat('error-avg : ',num2str(errors)),newline,horzcat('direc : ',num2str(direcs))],'Edgecolor','none');
        text_acc = "Accuracy by Validation until the latest GDO.";
        annotation('textbox',dim_2,'String',[text_acc;horzcat('mean-error: ',num2str(errors'));horzcat('direc-TF 2101~2105: ',num2str(direcs))],'Edgecolor','none');
    end
