...
for pindex = 1:47%:length(PrefVector)    
    ...
    for k = k_set:2
        ...
        for j=1:Tdata-TdataGDO
            ...
            rhos_2nd_j = ones(1,size(Xs_name_j,2));
            ...
            for varindex=1:size(Xs_name_j,2)
                ...
                if isnan(X_j(1))
                    ...
                    %最終的な残差のrho
                    r_p_star_j_lag = r_p_star_j(1:size(r_p_star_j,1)-1);
                    rho_2nd_j= (r_p_star_j_lag'*r_p_star_j_lag)\r_p_star_j_lag'*r_p_star_j(2:size(r_p_star_j,1));
                    rhos_2nd_j(:,varindex) = rho_2nd_j;
                ...
                if k==2
                    ...
                    Datas_struct.(pref).Result.(varname_j).rho_2nd = rho_2nd_j;
        ...
        str = horzcat(str,' ' );
            ...
            if j==1 %全部の変数が使えてるタイミングで、基本統計量を保持
                ...
                rho_2nd_table = table(rhos_2nd_j','RowNames',Xs_name_j);
            end
        ...
        if k==1
            clearvars -except stats today_date vintage_covid plot_other acc_plot acctest vintage_date vintage simple savedata Kasai k_set previous qrdei_pred pindex columns covid Datas_struct figure_save fn fs GDO_preds_star_simple home iPC Prefs_Vars PrefVector Result useful_seq;
        else
    ...
    %if pindex == 3
        ...
        if plot_mode == 1
            ...
            if stats == 1 
                ...
                %annotation('textbox','String',str_plot,'Edgecolor','none');%'FitBoxToText','on')
            end
        end
...
if strcmp(pref,'Yamanashi')
    ...
    if figure_save ==1
        saveas(gcf,strcat("Japan",'-'),'png');
        saveas(gcf,strcat("Japan"),'fig');
    end
