...
for pindex = 1:47%:length(PrefVector)    
    ...
    for k = k_set:2
        ...
        xtick1 = [1,4,7,10,13,16,19];
        year = 21; 
        month = Tdata-12; %2020年分の１２ヶ月を引く
        %xticklabel1 = horzcat([2001:2012],[2101:year*100+month;]);
        %xticklabel1 = horzcat(['Jan-2020',' ',' ', 04 05 06 07 08 09 10 11 12 01 02 03 04 05 06 07 08 09]);
        %xticklabel1 = horzcat(['Jan-2020', 'Jan-2020',' ',' ', 'Jan-2020',' ',' ','Jan-2020',' ',' ', 'Jan-2020',' ',' ', 'Jan-2020',' ',' ', 'Jan-2020',' ',' ',]);
        
        xticklabel1 = {'Jan-2020', 'Apr-2020', 'Jul-2020','Oct-2020', 'Jan-2021', 'Apr-2021', 'Jul-2021'};
    ...
    if mod(pindex,6) == 1 
        ...
        if pindex == 43
            Prefs_plot = PrefVector(pindex:pindex+4)
            
        else
            Prefs_plot = PrefVector(pindex:pindex+5)
        end
    end
    ...
    if mod(pindex,6) == 1 
        p1 = plot(GDO,'k-+','LineWidth',1.5); %3つ目
        hold on
        plot(GDO_pred,'k--+','LineWidth',1.5) %２つ目
    elseif mod(pindex,6) == 2
        p2 = plot(GDO,'k-o','LineWidth',1.5); %3つ目
        hold on
        plot(GDO_pred,'k--o','LineWidth',1.5)

    elseif mod(pindex,6) == 3 
        p3 = plot(GDO,'k-*','LineWidth',1.5); %3つ目
        hold on
        plot(GDO_pred,'k--*','LineWidth',1.5)
    elseif mod(pindex,6) == 4 
        p4 = plot(GDO,'k-x','LineWidth',1.5); %3つ目
        hold on
        plot(GDO_pred,'k--x','LineWidth',1.5)

    elseif mod(pindex,6) == 5
        p5 = plot(GDO,'k-s','LineWidth',1.5); %3つ目
        hold on
        plot(GDO_pred,'k--s','LineWidth',1.5)
    elseif mod(pindex,6) == 0 
        p6 = plot(GDO,'k-d','LineWidth',1.5); %3つ目
        hold on
        plot(GDO_pred,'k--d','LineWidth',1.5)
        hold on 
    end
    ...
    xtickangle(45)
    if mod(pindex,6) == 0
        legline = [p1,p2,p3,p4,p5,p6];
        %legend(legline,Prefs_plot,'FontSize',7,'Location','southeast');
        legend(legline,Prefs_plot,'FontSize',7,'Location','southeast');
    elseif pindex == 47
        legline = [p1,p2,p3,p4,p5];
        legend(legline,Prefs_plot,'FontSize',7,'Location','southeast');
    end
    ...
    %if pindex == 1 | pindex == 33 |pindex == 41  %saga 34
    if 0
          
        Data_vintage = vintage_covid.data(strcmp(vintage_covid.textdata(2:end,1),pref),:);
        vintage_mobility_raw = Data_vintage(:,17);
        vintage_mobility = normalize(vintage_mobility_raw);
        
        %過去の予測値のplot
        vintage_nowcasted = vintage.data(:,strcmp(vintage.textdata(1,1:end),pref)); %vintageが予測値自体、vintage_covidがcovid_monthlyに対応
        ...
        hold on
        plot(Fujii_raw,'k-.','LineWidth',1.5) %3つ目 %Fujii_rawはF-K GDP
        hold on
        
        %plot(naive_Fujii_raw,'b-.','LineWidth',1.5) %3つ目 %naive_Fujii_raw はFujii GDP
        %hold on
        ...
        plot(vintage_nowcasted,'g-.','LineWidth',1.5)
        ...
        if plot_other ==1 
            if k_set ~= 2
                ...
                legend('Mobility(L axis)','TDB(L axis)','Forecasted GDO (R axis)', 'Data Fujii (R axis)','Data GDO (R axis)','the other',strcat('vintage-',vintage_date),'qRDEI-pred (R axis)','FontSize',13,'Location','southeast');
            else
                legend('Mobility(L axis)','TDB(L axis)','Forecasted GDO (R axis)', 'Data Fujii (R axis)','Data GDO (R axis)','the other',strcat('vintage-',vintage_date),'FontSize',13,'Location','southeast');
            end
        else
            if k_set ~= 2 %%%基本ここを変えればOK
                plot(qrdei_pred,'c-.','LineWidth',1.5)
                legend('Mobility(L axis)','TDB(L axis)',strcat('vintage(mobility)-',vintage_date),'Forecasted GDO (R axis)', 'Data Fujii-Kasai (R axis)','Data Fujii(R axis)','Data GDO (R axis)',strcat('vintage-',vintage_date),'qRDEI-pred (R axis)','FontSize',13,'Location','southeast');
                %legend('Mobility(L axis)','TDB(L axis)','Forecasted GDO (R axis)', 'Data Fujii-Kasai (R axis)','Data Fujii(R axis)','Data GDO (R axis)',strcat('vintage-',vintage_date),'qRDEI-pred (R axis)','FontSize',13,'Location','southeast');
            else %%Kasaiがない時はこっち
                legend('Mobility(L axis)','TDB(L axis)',strcat('vintage(mobility)-',vintage_date),'Forecasted GDO (R axis)', 'Data Fujii (R axis)','Data GDO (R axis)',strcat('vintage-',vintage_date),'FontSize',13,'Location','southeast');
            end
        end
