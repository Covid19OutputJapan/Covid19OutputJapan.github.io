...
betaT_temp_ini_vec = [0.83, 0.37, -0.08];   %1.75 = 30000
betaT_rho_vec      = [0.75, 0.75, 0.75];
delta_ICU_nation_temp_ini_vec = [1,1,1]; %[0.9,0.9,3.0];
delta_ICU_pref_temp_ini_vec = [-0.5,-0.5,-0.5]; %[0,0,2.0];
delta_Hospital_temp_ini_vec = [4,4,4]; %[2.5,2.5,8.0];
th_on_vec          = ones(1,3)*max(BED)*0.4; %8000*7; %ICU_limit_pref_vec(end)*0.5; %
% th_on_vec          = ones(1,3)*max(Hospital_limit_vec)*0.5;
...
hospitalization_standard_vector = [1, 0.4, 0.2];
...
yvec = severity_nation_standard_vector;
...
markertype = {'o','o','o'};
lineNameJP = {"入院基準(基本)", "入院基準(やや厳格)", "入院基準(厳格)"};
lineNameEN = {"Baseline Hospitalization Standard", "Moderately Strict Hospitalization Standard", "Strict Hospitalization Standard"};
...
%             gamma_ICU_pref_path(1) = gamma_ICU_pref*gamma_shock_vec(iY);
            ...
            SimICU_nation_rate(2:end)   = SimICU_nation_rate(2:end) *  severity_nation_standard;
            SimHospital_rate(2:end)     = SimHospital_rate(2:end) *  hospitalization_standard;
            ...
            [DMat(iX,iY,iZ), AlphaMat(iX,iY,iZ), AlphaPath(:, iX,iY,iZ), ...
                ...
                hconstant,DRi, ...
                gamma_ICU_nation_path, gamma_ICU_pref_path, gamma_Hospital_path,...
...
disp(round(squeeze(NPath(1,1:3,1,1)/7)'))
...
for l = 1:2 %1:2 when english version needed
    ...
    for iX = 1:nX
        ...
        lng = language{l};
        figname = [char(figname_main) '_' num2str(Scenario_vec(iX,l),'%.2f') '_' char(lng)];
        f = figure('Name', figname);
