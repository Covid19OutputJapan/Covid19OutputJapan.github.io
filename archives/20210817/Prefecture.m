...
ICU_nation(end) = 1014; % 2021/08/09 %826; % 2021/08/02 % 678; % 2021/07/26
...
Nymax_vec = [20000,20000,20000,20000];

if Scenario_switch == 0
    Scenario = ["基本(楽観)", "基本", "基本(悲観)"];
    ScenarioEN = ["Baseline (Optimistic)", "Baseline", "Baseline(Pessimistic)"];
    beta_scale_vec = ones(1, 3) * 0.98; %ones(1,3) * 0.875;
    xline_ind2 = find(date == datetime(2021, 10, 14)); % consider the case where state of emergency is extended
    figname_main = 'Baseline_MainResults_Sep';
...
elseif Scenario_switch == 1
    Scenario = ["Alternative (楽観)", "Alternative", "Alternative(悲観)"];
    ScenarioEN = ["Alternative (Optimistic)", "Alternative", "Alternative(Pessimistic)"];
    beta_scale_vec = ones(1, 3) * 1.1;
    figname_main = 'Alternative_MainResults_Sep';
    xline_ind2 = find(date == datetime(2021, 10, 14)); % consider the case where state of emergency is extended
...
elseif Scenario_switch == 2
    Scenario = ["ロックダウンシナリオ1 (楽観)", "ロックダウンシナリオ1", "ロックダウンシナリオ1(悲観)"];
    ScenarioEN = ["Lockdown Scenario 1 (Optimistic)", "Lockdown Scenario 1", "Lockdown Scenario 1(Pessimistic)"];
    lockdown_th = 0.8;
    ...
    figname_main = 'Lockdown_MainResults_80';
    figname_beta_tilde = 'Lockdown Scenario 1 Beta Tilde Path';
    figname_beta = 'Lockdown Scenario 1 Beta Path';
    figname_var = 'Lockdown Scenario 1 Variant_Share';
    figname_delta = 'Lockdown Scenario 1 Death Rate transition';
    figname_ICU_nation = 'Lockdown Scenario 1 ICU Rate transition National Standard';
    figname_ICU_local = 'Lockdown Scenario 1 ICU Rate transition Local Standard';
    figname_ERN = 'Lockdown Scenario 1 ERN transition';
    figname_Hospital = 'Lockdown Scenario 1 Hospital Rate transition';
    figname_dD = 'Lockdown Scenario 1 dD transition';
elseif Scenario_switch == 3
    Scenario = ["ロックダウンシナリオ2(楽観)", "ロックダウンシナリオ2", "ロックダウンシナリオ2(悲観)"];
    ScenarioEN = ["Lockdown Scenario 2 (Optimistic)", "Lockdown Scenario 2", "Lockdown Scenario 2 (Pessimistic)"];
    lockdown_th = 0.5;
...
for iTH = 1:length(TH)
    ...
    [delta_wo_alpha, delta_wo_vaccination, delta_ICU_nation_wo_vaccination, ...
        ...
        beta_avg, beta_wo_alpha, beta_se ...
        ...
        beta, D1, D2, V1_elderly, V2_elderly, V1_medical, V2_medical, V1_others, V2_others, ...
...
%     beta_se = 0;
    ...
    elseif Scenario_switch == 2 %ロックダウン70%
        beta_jump = 0.75;
        ...
        %         betaT(2:9) =  betaT(2:9);
        %         betaT(1) = betaT(1);
        %         betaT(2) = betaT(2); %1.7582;
        %         betaT(3) = betaT(3);
        %         betaT(1) = betaT(1) * 0.9;
        %         betaT(2) = betaT(2) * 1.0813; %1.7582;
        %         betaT(3) = betaT(3) * 1.0486;
        %         betaT(4:xline_ind - Tdata) = beta_scale * betaT(4:xline_ind - Tdata);
        %         betaT(xline_ind - Tdata + 1:xline_ind2 - Tdata) = 2.2;
...
%         betaT(th_off_date + DRi + 1:end) = betaT(th_off_date + DRi + 1:end) + beta_additive_path(end);
    ...
    elseif Scenario_switch == 3 %ロックダウン50%
        ...
        %          betaT(2:end) = beta_scale * betaT(2:end);
        
        %         betaT(2:9) = beta_scale * betaT(2:9);
        % %         betaT(1) = betaT(1);
        % %         betaT(2) = betaT(2); %1.7582;
        % %         betaT(3) = betaT(3);
        % %         betaT(1) = betaT(1) * 0.9;
        % %         betaT(2) = betaT(2) * 1.0813; %1.7582;
        % %         betaT(3) = betaT(3) * 1.0486;
        %         %         betaT(3:xline_ind - Tdata) = beta_scale * betaT(3:xline_ind - Tdata);
        %         %         betaT(xline_ind - Tdata + 1:xline_ind2 - Tdata) = 2.2;
        %         betaT_ref = betaT;
        %         beta_jump = 0.5; %0.25; %0.25;
        %         % beta_additive_path = transpose(0 + (beta_goal) * beta_jump + (1 - beta_jump) * ((beta_goal) / (DRi + 1)):(1 - beta_jump) * ((beta_goal) / (DRi + 1)):beta_goal);
        %         beta_additive_path = transpose(0 + (beta_goal) * beta_jump:(1 - beta_jump) * ((beta_goal) / (DRi)):beta_goal);
        %         betaT_ref(th_off_date:th_off_date + DRi) = betaT(th_off_date:th_off_date + DRi) + beta_additive_path;
        %         betaT_ref(th_off_date + DRi + 1:end) = betaT(th_off_date + DRi + 1:end) + beta_additive_path(end);
...
%         
    ...
    % beta_goal = 0.025 * 30; %0.6; %0.75;
    % beta rise at 0.025 by 1% increase of mobility
    %     betaT_ref = betaT;
    %     beta_jump = 0.5; %0.25; %0.25;
    %     % beta_additive_path = transpose(0 + (beta_goal) * beta_jump + (1 - beta_jump) * ((beta_goal) / (DRi + 1)):(1 - beta_jump) * ((beta_goal) / (DRi + 1)):beta_goal);
    %     beta_additive_path = transpose(0 + (beta_goal) * beta_jump:(1 - beta_jump) * ((beta_goal) / (DRi)):beta_goal);
    ...
    elseif Scenario_switch == 2 || Scenario_switch == 3
%         BED(end) = BED(end-1);
...
for l = 1:2 %1:2 when english version needed
    ...
    if Scenario_switch <= 1 %|| Scenario_switch == 1 
%         xline(xline_ind, 'LineWidth', 1.5, 'HandleVisibility', 'off');
    ...
    text(xmin, ICU_limit_pref_vec(xmin) * 0.9, '100%', 'FontSize', fs)
    hold on
    plot([ICU_limit_pref_vec * 2; ones(SimPeriod, 1) * ICU_limit_pref_vec(end) * 2], '--k', 'HandleVisibility', 'off', 'LineWidth', 1.5)
...
%     end
    ...
    ylim([0 ICU_limit_pref_vec(end) * 3 * 1.1]) %ylim([0 ICU_limit * 1.1])
...
%     end
    ...
    xlim([xmin, xmax])
    ylim([0 BED(end) * 3 * 1.1]) %ylim([0 2500])
    Handle = legend;
