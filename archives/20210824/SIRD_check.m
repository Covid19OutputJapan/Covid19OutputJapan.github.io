...
% plot(I(2:end),'red','lineStyle','--','LineWidth',1.5,'DisplayName','療養者(model)')
%%
figure('Name','Plot I and dD')
set(gcf,'Position',[100,100,1000,600])
X = I_data;
Y = dD_data;
for lag = 0:5
    subplot(2,3,1+lag)
    scatter(X(2:end-lag),Y(1+lag:end))
    title([sprintf(yft,lag) 'lag'])
    xlabel('I')
    ylabel('dD')
end

figure('Name','Plot ICU local and dD')
set(gcf,'Position',[100,100,1000,600])
X = ICU_pref;
Y = dD_data;
for lag = 0:5
    ...
    scatter(X(2:end-lag),Y(1+lag:end))
    title([sprintf(yft,lag) 'lag'])
    xlabel('ICU local standard')
    ylabel('dD')
end

figure('Name','Plot I and ICU (Local Standard)')
set(gcf,'Position',[100,100,1000,600])
X = I_data;
Y = ICU_pref;
for lag = 0:5
    subplot(2,3,1+lag)
    scatter(X(2:end-lag),Y(2+lag:end))
    title([sprintf(yft,lag) 'lag'])
    xlabel('I')
    ylabel('dD')
end

figure('Name','Plot I and ICU (National Standard)')
set(gcf,'Position',[100,100,1000,600])
X = I_data;
Y = ICU_nation;
for lag = 0:5
...
maxLag = 10;
lags = 0:maxLag;
ILags = lagmatrix(I_data(2:end),lags);
lagNames = strcat({'I_Lag'},num2str(lags','%-d'));
ILags = array2table(ILags,'VariableNames',lagNames);

DTAL = cell(maxLag+1,1);
MAL = cell(maxLag+1,1);
% Fit models:
AL = ILags;
yNames = strcat({'dD'});
DT = array2table(dD_data,'VariableNames',yNames);
...
% X = ones(Tdata,nLag+1);
% for iLag = 1:nLag
%     X(:,iLag+1) = [zeros(iLag-1,1);x(2:end-(iLag-1))];
% end
% b = regress(y,X);
% 
% y = dD_data;
