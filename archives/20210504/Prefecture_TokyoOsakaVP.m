...
% 緊急事態宣言の発令基準
th_on_vector = [750,500,400,350,1200,350,350]; % present.
th_on_vector2 = [1750,1000,800,700,2000,350,350]; % 高齢者がうち終わったあとの基準

% 緊急事態宣言の解除基準
th_off_vector = [700,100,110,80,1060,50,60]; %1回目の緊急事態宣言の解除基準
th_off_vector2 = [500,100,80,60,700,50,60]; %2回目の緊急事態宣言の解除基準
th_off_vector3 = [1000,100,80,60,500,50,60]; %3回目の緊急事態宣言の解除基準
% 経済回復速度
...
betaT_temp_ini_vec = [0.5,0,0,0,0,0,0,0];%0.15
...
for pindex = 5:5 %:length(PrefVector) %change this parameter for prefecture
...
%                 (InitialValues,alpha_on,alpha_off,th_on1,th_on2,th_off1,th_off2,th_off3,beta_avg*ones(SimPeriod,1),gammaT,deltaT,delta_ss,V,h,k,POP0,hconstant,DRi);
    ...
    for l = 1:2 %1:2 when english version needed
        ...
        [BackDataN,BackDataAlpha,BackDataERN] ...
            = plot_SimN2(TH,TH_index,N,NPath,alpha,AlphaPath,ERN,SimERN,MonthWeekEN,MonthWeekJP,WeekNumber,Tdata,linecolor,20,fs,fn,ft,l);
