...
% ==================== CHANGE HERE ====================== %
% ======================================================= %
% Control on the vaccine setup
TH = [6300000,4900000,7000000];
TH_index = TH;
gradual_paces_vector = [1,1,1];
sw_vacpath_vec = [0,0,0];
ind_date_vec = [find(date == datetime(2021,7,22)),find(date == datetime(2021,7,29)),find(date == datetime(2021,7,22))]; %Date by which the first shots should finish
% vaccine paces for shokuiki 
total_shots_vec = [8400000,7000000,9800000]; 
paces2_ori_vec = total_shots_vec - TH;
gradual_paces2_vec = [2,1,2];
paces3_vec = [6300000,4200000,7000000];
accept_share_vec = [0.8,0.7,0.8];

lagged_VT2share = 0.9; % how many percent of people get their 2nd doses in "lag" weeks
% lagged_VT2share in the last 6 wks = mean(V2_elderly(end-6+1:end)./V1_elderly(end-9+1:end-3)) =  0.8903
VT3share_vec = [0.6,0.6,0.6];
lag_vec = [3,3,3];
medical_duration_vec = [1,1,1];
...
%     var_initial2 = 0.13
    ...
    %alpha_scale = 0.94;
    alpha_scale = 1.0;
    alpha_on = (0.25 * alpha_May + 0.75 * alpha_Jan); %alpha_scale * (0.5 * alpha_May + 0.5 * alpha_Jan);
    h_scale = 1;
...
elseif pindex == 5
    ...
    h_scale = 1;
    beta_shock_after_emergency = 0.89;  % beta = 1.5633 (average of 2020 December and 2021 April)
    rho_after_emergency = 0.65;
    %alpha_jump = 0;
    alpha_jump = (alpha(end)-alpha_on)/(alpha_off - alpha_on);
end
h = h_ori;
...
title_vec = {'基本','悲観','希望'};
title_EN_vec = {'Baseline','Pessimistic','Optimistic'};
...
for iTH = 1:length(TH)
    ...
    ind_date = ind_date_vec(iTH) - 1;
    VT3share = VT3share_vec(iTH);
    lag = lag_vec(iTH);
    medical_duration = medical_duration_vec(iTH);
    paces2_ori = paces2_ori_vec(iTH);
    paces2 = paces2_ori*ps;
    gradual_paces2 = gradual_paces2_vec(iTH);
    paces3 = paces3_vec(iTH)*ps;
    accept_share = accept_share_vec(iTH);
...
%     writematrix(VT,['\Users\masam\Desktop\VT_' num2str(TH(iTH)) '_' char(pref) '.csv']);

    % Remove variant effects from delta_average and construct deltaT and deltaICU 
    ...
    if vaccine_figure_loop == 0
        if iTH == 1
            figname_V = 'Vaccine Path';
%             plot_vaccine(figname_V,VT,V1_medical,V2_medical,V1_elderly,V2_elderly,date,MonthWeekJP,WeekNumber,Tdata,fs,ldfs,fn);
    ...
    elseif vaccine_figure_loop == 1
        figname_V = ['Vaccine Path Paces = ' sprintf(ft,paces_ori)];
%         plot_vaccine(figname_V,VT,V1_medical,V2_medical,V1_elderly,V2_elderly,date,MonthWeekJP,WeekNumber,Tdata,fs,ldfs,fn);
...
%         title(string(['新規ワクチン接種本数（週ごと）（',sprintf(ft,TH(iTH)),'）']), 'FontSize',fs,'FontName',fn);
        title(string(['新規ワクチン接種本数（' title_vec{iTH} '）']), 'FontSize',fs,'FontName',fn);
%         ylim([0 ps*paces_ori*1.2/10000])
        subplot(1,2,2)
%         title(string(['累計ワクチン接種本数（週ごと）（',sprintf(ft,TH(iTH)),'）']), 'FontSize',fs,'FontName',fn);
        title(string(['累計ワクチン接種本数（' title_vec{iTH} '）']), 'FontSize',fs,'FontName',fn);
    ...
    if figure_save == 1
        saveas(figure(200+iTH), [home 'Figures/' char(pref) '/Vaccine_path_' title_EN_vec{iTH} '.png']);
    end

    if figure_save == 1 && iTH == length(TH)
        saveas(figure(240), [home 'Figures/' char(pref) '/Death_rate.png']);
        saveas(figure(250), [home 'Figures/' char(pref) '/ICU_rate.png']);
    end
...
for l = 1:2 %1:2 when english version needed
    ...
    subplot(2,2,1)
    [BackDataN,BackDataAlpha,BackDataERN] = plot_SimN2(TH,TH_index,N,NPath,alpha,AlphaPath,ERN,SimERN,THonPath,MonthWeekEN,MonthWeekJP,WeekNumber,Tdata,linecolor,fs,ldfs,fn,ft,l);
    ...
    if ICU_nation == 1
        ...
        BackDataICU = plot_ICU(TH,TH_index,ICU,SimICU,ICU_limit,MonthWeekEN,MonthWeekJP,WeekNumber,Tdata,linecolor,ldfs,fn,ft,l,th_off1);
    end
    subplot(2,2,4)
...
% 
% subplot(2,2,1)
% plot_SimN2(TH,TH_index,N,NPath,alpha,AlphaPath,ERN,SimERN,THonPath,MonthWeekEN,MonthWeekJP,WeekNumber,Tdata,linecolor,fs,ldfs,fn,ft,l)
% subplot(2,2,2)
...
eTitle = 'Transitions of Death Rate and ICU Rate';
jTitle = '死亡率と重症化率の推移';
plot_function2(TH,TH_index,delta,deltaPath,...
    delta,deltaICUPath,MonthWeekEN,MonthWeekJP,WeekNumber,Tdata,xline_ind,linecolor,lineName,...
    lineName2,eTitle,jTitle,fs,lgfs,fn,ft,yft,column,l,show_other,'SouthOutside')
...
% xlim([Tdata-31 Tdata+41])
% %%
% % ====== Plot Vaccine Path for Japan ===== %
% vaccine_medical_jp = readmatrix([home 'vaccine_daily_medical.xls']);
% vaccine_elderly_jp = readmatrix([home 'vaccine_daily_elderly.xls']);
% [V1_medical_jp, V2_medical_jp] = vaccine_daily_to_weekly_table(vaccine_medical_jp, 1, dateEN,iPC);
% [V1_elderly_jp, V2_elderly_jp] = vaccine_daily_to_weekly_table(vaccine_elderly_jp, 1, dateEN,iPC);
% V_jp = zeros(SimPeriod,nTH);
% VT_jp = zeros(SimPeriod,6,nTH);
% for iTH = 1:length(TH)
%     paces_ori = TH(iTH);
%     gradual_paces = gradual_paces_vector(iTH);
%     sw_vacpath = sw_vacpath_vec(iTH);
...
%     medical_duration = medical_duration_vec(iTH);
%     [V_jp(:,iTH),~,VT_jp(:,:,iTH),~] = ...
%         vaccine_distribution(V1_medical_jp,V2_medical_jp,...
%         V1_elderly_jp,V2_elderly_jp,ind_date,VT3share,...
%         elderly_jp*accept_share,elderly_jp,medical_jp,ordinary_jp*accept_share,delta_average,lag,...
%         medical_duration,paces_ori,sw_vacpath,gradual_paces,...
%         E1,E2,D1,D2,1,POP0,SimPeriod,Tdata);
% figname_Vjp = ['Vaccine Path Japan, ' sprintf(ft,paces_ori)];
% plot_vaccine(figname_Vjp,VT_jp(:,:,iTH),V1_medical_jp,V2_medical_jp,V1_elderly_jp,V2_elderly_jp,date,MonthWeekJP,WeekNumber,Tdata,fs,ldfs,fn);
