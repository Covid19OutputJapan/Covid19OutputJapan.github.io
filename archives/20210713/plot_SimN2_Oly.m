function [BackDataN, BackDataAlpha, BackDataERN]...
    = plot_SimN2(TH,TH_index,N,NPath,alpha,AlphaPath,ERN,SimERN,THonPath,MonthWeekEN,MonthWeekJP,WeekNumber,Tdata,linecolor,fs,fs_legend,fn,ft,l,OlympicStart,OlympicEnd,ParaStart,ParaEnd)
...
a = fill([OlympicStart, OlympicStart,OlympicEnd,OlympicEnd],[0,3000,3000,0],[0, 0, 1]);
a.FaceAlpha = 0.1;
b = fill([ParaStart, ParaEnd,ParaEnd,ParaStart],[0,3000,3000,0],[0, 0, 1]);
b.FaceAlpha = 0.1;
if l == 1
    ...
    legend([mainplot(1),mainplot(2)], 'Baseline','Optimistic','FontSize',fs_legend,'Location','northwest');
elseif l == 2
    title('新規感染者数の推移','FontSize',fs,'FontWeight','normal','FontName',fn)
    xticklabels(MonthWeekJP(WeekNumber==1))
    legend([mainplot(1),mainplot(2)], '基本見通し','希望見通し','FontSize',fs_legend,'FontName',fn,'Location','northwest');
end
