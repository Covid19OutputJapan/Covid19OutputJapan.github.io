...
if iPC == 1
    home = '\Users\Hiroaki\Dropbox\fujii_nakata\Website\Codes\';
else
...
delta_average = delta_average/share; %Eliminate the effects of vaccination in the past average value
delta_wo_vaccination = delta_average; %Past 17 without vaccine effects
...
ICU_wo_vaccination = delta_past_avg/share; %Eliminate the effects of vaccination in the past average value
...
var_infection_adjustment = (1-mean(var_prev(end-17+1:end)))*1 ...
    + mean(var_prev(end-17+1:end))*(1 + var_infection); %Relative increase of infectiousness (alpha varaint, past 17 weeks)
var_infection_delta_adjustment = (1-mean(var_prev(end-17+1:end)))*1 ...
    + mean(var_prev(end-17+1:end))*(1 + var_infection_delta); %Relative increase of death rate (alpha varaint, past 17 weeks)

delta_average = delta_average/var_infection_delta_adjustment; %Eliminate the effects of alpha variant in the past average value
delta_wo_alpha = delta_average;

ICU_wo_alpha = ICU_wo_vaccination/var_infection_delta_adjustment;

beta_bar = beta_avg; %Past weeks average
beta_avg = beta_avg/var_infection_adjustment; %Eliminate the effects of alpha variatn
beta_wo_alpha = beta_avg;
...
%TH =  [0.2, 0.3,0.4];%[0.1, 0.2,0.3];relative infectivity of the delta
%variant
TH=[0.735,0.82,0.88];%beta scale
SoE_TH_off=[600,800,970];
...
if pindex == 1
    ...
    %alpha_scale = 0.94;
    alpha_scale = 1.0;
    alpha_on = alpha_Jan;%(0.5 * alpha_May + 0.5 * alpha_Jan); %alpha_Jan; %alpha_scale * (0.5 * alpha_May + 0.5 * alpha_Jan);
    h_scale = 1;
...
for iTH = 1:length(TH)
    ...
    var_infection_adjustment2 = (1-mean(var_prev2(end-17+1:end)))*1 ...
        + mean(var_prev2(end-17+1:end))*(1+var_infection2); %Relative increase of infectiousness (alpha varaint, past 17 weeks)
    var_infection_delta_adjustment2 = (1-mean(var_prev2(end-17+1:end)))*1 ...
        + mean(var_prev2(end-17+1:end))*(1 + var_infection_delta2); %Relative increase of death rate (alpha varaint, past 17 weeks)
    ...
    ICU_average = ICU_wo_alpha/var_infection_delta_adjustment2; 
...
%     betaT = beta_AR1(betaT_temp_ini, beta_rho, betaT, start_beta);
    ...
    betaT_woAR = betaT;
    betaT = beta_AR1(betaT_temp_ini, beta_rho, betaT, start_beta);
    betaT(1)=betaT(1)*1.29;
    betaT(2)=betaT(2)*1.125;
    betaT(3:6)=beta_scale(iTH)*betaT(3:6);
...
for l = 1:2 %1:2 when english version needed
    ...
    if ICU_nation == 1
        BackDataICU = ...
            plot_ICU_GOV([600,800, 1000],[600, 800, 1000],ICU,SimICU,BED(Tdata),...
...
if data_save == 1
    ...
    for ti = 1:length(TH_index)
        ...
        titleN(1,1+length(TH_index)*3+ti) = string(['重症者数（',sprintf(ft,TH_index(ti)),'）']);
    end
    TN = table([titleN;MonthWeekJP(Tdata-7:end-1),round(BackDataN(:,1:length(TH_index))/7),round(100*(1-BackDataAlpha(:,1:length(TH_index))),1),round(BackDataERN(:,1:length(TH_index)),2),round(BackDataICU(:,1:length(TH_index)))]);
...
for i = 1:nTH
    lineName{i} = ['I, 相対感染率上昇 : ' sprintf(ft, TH(i))];
    lineName2{i} = ['N, 相対感染率上昇 : ' sprintf(ft, TH(i))];
end
...
lineName2 = cell(nTH,1);
for i = 1:nTH
    lineName{i} = ['S, 相対感染率上昇 :  ' sprintf(ft, TH(i))];
    lineName2{i} = ['R, 相対感染率上昇 :  ' sprintf(ft, TH(i))];
end
...
lineName = cell(nTH,1);
for i = 1:nTH
    lineName{i} = ['相対感染率上昇 :  ' sprintf(ft, TH(i))];
end
...
figname_ICU = 'ICU Rate transition';
figure('Name',char(figname_ICU));
set(gcf,'Position',[100,100,1200,800])
eTitle = 'Transitions of ICU Rate';
jTitle = '重症化率の推移';
plot_function(TH,TH_index,delta.*ICU_inflow,ICU_inflow_avg*deltaICUPath,...
    MonthWeekEN,MonthWeekJP,WeekNumber,Tdata,xline_ind,linecolor,lineName,...
    eTitle,jTitle,fs,lgfs,fn,ft,yft,column,l,show_other,'SouthOutside')
plot([NaN(Tdata - 1, 1); ones(SimPeriod + 1, 1) * delta_past_avg*ICU_inflow_avg], '--k', 'LineWidth', 1.5, 'DisplayName', '過去17週間平均')
plot([NaN(Tdata - 1, 1); ones(SimPeriod + 1, 1) * ICU_wo_vaccination*ICU_inflow_avg], '--b', 'LineWidth', 1.5, 'DisplayName', '過去17週間平均, ワクチン影響除去')
plot([NaN(Tdata - 1, 1); ones(SimPeriod + 1, 1) * ICU_wo_alpha*ICU_inflow_avg], ':r', 'LineWidth', 1.5, 'DisplayName', '過去17週間平均, ワクチン影響除去+アルファ株影響除去')
plot([NaN(Tdata - 1, 1); ones(SimPeriod + 1, 1) * ICU_average*ICU_inflow_avg], '--r', 'LineWidth', 1.5, 'DisplayName', '過去17週間平均, ワクチン影響除去+変異株影響除去')
xline(Tdata-RetroPeriodDelta-1,'HandleVisibility','off')
xlim([Tdata-31 Tdata+41])
if figure_save == 1
