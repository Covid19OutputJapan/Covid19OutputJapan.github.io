function BackDataICU = ...
    plot_ICU(TH,TH_index,ICU,SimICU,ICU_limit,MonthWeekEN,MonthWeekJP,WeekNumber,Tdata,linecolor,fs,fn,ft,l,th_off1)
th_wave = 1;
for i = 1:length(TH)
    if abs(TH(i) - TH_index(th_wave)) < 0.0001        
        p = plot([ICU(2:end);SimICU(2:end,i)],linecolor{th_wave},'LineWidth',2,'DisplayName',sprintf(ft,TH(i)));
        p.LineStyle = ':';
%         hold on 
