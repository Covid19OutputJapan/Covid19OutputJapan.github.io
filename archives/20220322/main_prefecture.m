...
elseif ismac == 1
    HOME        = '/Users/sohtakawawaki/Documents/GitHub/github_Codes/'; %include \ or / at the end
    DATA_PATH   = '/Users/sohtakawawaki/Dropbox/fujii_nakata (1)/Website/Codes/Data/';
    FIGURE_PATH = '/Users/sohtakawawaki/Dropbox/fujii_nakata (1)/Website/Codes/Figures/';
    FONT_NAME          = 'YuGothic';
end
cd(HOME);
...
T2_ind = find(date == datetime(2022, 3, 3));
...
%iY
delta_temp_ini_vec = [0.35, 0.35, 0.35];
delta_rho_vec = [0.8, 0.8, 0.8];

delta_ICU_nation_temp_ini_vec =[-0.3, -0.3,-0.3]; %[0.9,0.9,3.0];
delta_ICU_pref_temp_ini_vec = [-0.3, -0.3, -0.2]; %[0,0,2.0];

delta_newICU_pref_temp_ini_vec = [-0.5, -0.5, -0.5]; %[0,0,2.0];
delta_Hospital_temp_ini_vec = [-0.3, -0.3, -0.3]; %[2.5,2.5,8.0];

target_relative_severity = [0.25 ,0.2, 0.15];
...
%         beta_jump_path(SoE_off_date+beta_jump_duration+1:end)              = ones(size(beta_jump_path(SoE_off_date+beta_jump_duration+1:end)));
        ...
        delta_temp_ini = delta_temp_ini_vec(iY);
        delta_rho = delta_rho_vec(iY);
...
%                     + (1 - beta_jump_path(SoE_off_date+1:end)) .* betaT(SoE_off_date);
            ...
            alphaT          = [ones(alpha_date_index,1)*alpha_recovery_initial; alphaAfterSOE(2:end)';alpha_off * ones(SimPeriod-alpha_date_index+1,1)];
