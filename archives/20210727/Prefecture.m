...
% Nakata
% For Optimistic Scenario, set Optimistic_switch = 1.
...
if iPC == 1
    ...
    %     home = '/Users/MEIP-users/Dropbox/fujii_nakata/Website/Codes_0725/';
else
...
% Optimistic_switch = 1; % To simulate pessimistic case, choose Optimistic_switch = 0
% linecolor = {'red', 'red', 'red', 'blue', 'blue', 'blue'};
Optimistic_switch = 0;
if Optimistic_switch == 1
...
% V1_elerly = (V1_elderly_old + V1_others_old) * (sum(V1_elderly)/sum(V1_elderly + V1_others));
% V1_others = (V1_elderly_old + V1_others_old) * (sum(V1_others)/sum(V1_elderly + V1_others));
% V2_elerly = (V2_elderly_old + V2_others_old) * (sum(V2_elderly)/sum(V2_elderly + V2_others));
% V2_others = (V2_elderly_old + V2_others_old) * (sum(V2_others)/sum(V2_elderly + V2_others));

% V1_medical(end) = V1_medical(end)*7/4; @if not full data for the last
% week is availale
...
if Optimistic_switch == 1
    Scenario = ["楽観(1.5 std)", "楽観" ,"楽観(-1.5 std)"];
    ScenarioEN = ["Optimistic (1.5 std)", "Optimistic" ,"Optimistic(-1.5 std)"];
    beta_scale_vec = ones(1,3) * 0.875;
    xline_ind2 = find(date == datetime(2021,9,23)); % consider the case where state of emergency is extended
    figname_main = 'Optimistic_MainResults';
    
    figname_beta_tilde = 'Optimistic Beta Tilde Path';
    figname_beta = 'Optimistic Beta Path';
    figname_var = 'Optimistic Variant_Share';
    figname_delta = 'Optimistic Death Rate transition';
    figname_ICU_nation = 'Optimistic ICU Rate transition National Standard';
    figname_ICU_local = 'Optimistic ICU Rate transition Local Standard';
    figname_ERN = 'Optimistic ERN transition';
    figname_Hospital = 'Optimistic Hospital Rate transition';
    figname_dD = 'Optimistic dD transition';
else
    Scenario = ["悲観(1.5 std)", "悲観" ,"悲観(-1.5 std)"];
    ScenarioEN = ["Pessimistic (1.5 std)", "Pessimistic" ,"Pessimistic(-1.5 std)"];
    beta_scale_vec = ones(1,3) * 0.89;
    xline_ind2 = find(date == datetime(2021,9,23)); % consider the case where state of emergency is extended
    figname_main = 'Pessimistic_MainResults';
    
    figname_beta_tilde = 'Pessimistic Beta Tilde Path';
    figname_beta = 'Pessimistic Beta Path';
    figname_var = 'Pessimistic Variant_Share';
    figname_delta = 'Pessimistic Death Rate transition';
    figname_ICU_nation = 'Pessimistic ICU Rate transition National Standard';
    figname_ICU_local = 'Pessimistic ICU Rate transition Local Standard';
    figname_ERN = 'Pessimistic ERN transition';
    figname_Hospital = 'Pessimistic Hospital Rate transition';
    figname_dD = 'Pessimistic dD transition';
...
xline_ind = find(date == datetime(2021, 8, 19)); % when the state of emergency end
...
for iTH = 1:length(TH)
    ...
    % ========== Change here ==================== %
    if Optimistic_switch == 1 %Optimistic
        betaT(1) = betaT(1) * 1.3498;
        betaT(2) = betaT(2) * 1.1937;
        betaT(3:th_off_date) = beta_scale * betaT(3:th_off_date);
    elseif Optimistic_switch == 0 %Pessimistic
        betaT(1) = betaT(1) * 1.3498;
        betaT(2) = betaT(2) * 1.2931;
        betaT(3) = betaT(3) * 1.1788;
        betaT(4:th_off_date) = beta_scale * betaT(4:th_off_date);
    end
...
for l = 1:2 %1:2 when english version needed
    ...
    subplot(3, 3, 1)
    BackDataICU_pref = ...
        plot_ICU(TH, TH_index, ICU_pref, SimICU_pref, ICU_limit, MonthWeekEN, MonthWeekJP, WeekNumber, Tdata, linecolor, fs, fn, ft, l,lineName);
    ...
    BackDataICU_nation = ...
        plot_ICU(TH, TH_index, ICU_nation, SimICU_nation, BED(Tdata), MonthWeekEN, MonthWeekJP, WeekNumber, Tdata, linecolor, fs, fn, ft, l,lineName);
    xline(xline_ind2, 'LineWidth', 1.5, 'HandleVisibility', 'off');
    hold on
