...
clear variables
close all
home = '/Users/Daisuke/Desktop/Dropbox/Research/fujii_nakata/Website/Codes/OneWeekAgo/20210120/';
 %home = pwd;
...
% if LoadNext == 1
NextD = load('Japan20210126');
alphaNew = NextD.alpha(end);
dDActualNew = NextD.D(end)-NextD.D(end-1);
NActualNew = NextD.N(end);
 [CumDNew,AverageAlphaNew,SimDataNew,NForecastNew]=Covid_projection(InitialValues,alphaNew,betaT,gammaT,deltaT,V,h,k,POP0,hconstant);
dDForecastNew = CumDNew-D(end);

CurrentD = load([home 'Japan_0121_2021']);
dNActual = [CurrentD.dNActual(:,1);NActualNew];
dDActual = [CurrentD.dDActual(:,1);dDActualNew];
dNForecast = [CurrentD.dNForecast(:,1);NForecastNew];
dDForecast = [CurrentD.dDForecast(:,1);dDForecastNew];

disp('dDForecast(end)')
disp(dDForecast(end))
disp('dDActual(end)')
disp(dDActual(end))
disp('dDForecast(end)-dDActual(end)')
disp(dDForecast(end)-dDActual(end))
disp('dNForecast(end)')
disp(dNForecast(end))
disp('dNActual(end)')
disp(dNActual(end))
disp('dNForecast(end)-dNActual(end)')
disp(dNForecast(end)-dNActual(end))
...
xtick1_F=[StartDateF+4:8:(Tdata-1),(Tdata-1)];
xtick4_F=[StartDateF+4:8:(Tdata-1),(Tdata-1)];
fs_F=10;
fs_legend_F=10;
fs_title=12;
figure(10)
...
ax.YAxis.Exponent = 0;
ytickformat('%,6.0f')
%saveas(figure(10),[home 'ForecaseErrors.png']);


figure(11)
subplot(2,2,1)
plot(StartDateF:(Tdata),dNForecast(StartDateF:Tdata,1),'r',StartDateF:(Tdata),dNActual(StartDateF:Tdata,1),'k','LineWidth',1.5)  
title({'??????';'????????'},'FontSize',fs_title,'FontWeight','normal')
xlim([StartDateF (Tdata)])
xticks(xtick1_F)
xticklabels(Month(xtick1_F))
ax = gca;
...
ax.XAxis.FontSize = fs_F;
ytickformat('%,6.0f')
legend('??','???','FontSize',fs_legend_F,'Location','Northwest')
subplot(2,2,2)
plot(StartDateF:(Tdata),dDForecast(StartDateF:Tdata,1),'r',StartDateF:(Tdata),dDActual(StartDateF:Tdata,1),'k','LineWidth',1.5)  
title({'??????';'????????'},'FontSize',fs_title,'FontWeight','normal')
ax = gca;
...
ax.YAxis.Exponent = 0;
ytickformat('%,6.0f')
%saveas(figure(11),[home 'ForecaseErrors_jp.png']);
