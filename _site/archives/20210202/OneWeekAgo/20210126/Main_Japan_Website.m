...
for l = 1:2
    ...
    for j = 1:4
        ...
        for i = 1:length(AlphaVals)
            ...
            elseif l == 2
                xticklabels( {'20?1?','20?7?','21?1?','21?7?','21?12?'} )
            end
            xtickangle(45)
            if j == 1
...
% NextD = load('/Users/Daisuke/Desktop/Dropbox/Research/fujii_nakata/Website/Codes/Japan20210202.mat');
% alpha = NextD.alpha;
...
% dNFNew = dNForecast(end,:);
% dDFNew = dDForecast(end,:);
% dNANew = dNActual(end,:);
% dDANew = dDActual(end,:);
% 
% save('/Users/Daisuke/Desktop/Dropbox/Research/fujii_nakata/Website/Forecast.mat','dNFNew','dDFNew','dNANew','dDANew','-append');
...
elseif iDrawUB==1
    ...
    LastD = load('../Covid19OutputJapan.github.io/archives/20210120/Japan_0121_2021');
    ...
    for l = 1:2
        ...
        elseif l == 2
            plot(X1UB,CumD,'k','LineWidth',2,'DisplayName','??')
            hold on
            % result from last week with current specification %
            wlag = 1;
            plot(100*LagResults(2,:,1),LagResults(1,:,1),'r--','LineWidth',1.5,'DisplayName','???????????')
            hold on
            plot(100*LastD.AverageAlpha,LastD.CumD,'r','LineWidth',1.5,'DisplayName','????')
            hold on
            plot(100*LastD.LagResults(2,:,1),LastD.LagResults(1,:,1),'b','LineWidth',1.5,'DisplayName','????')
        end
        % for i = 1:length(wl)
        %     wlag = wl(i);
        %     if wlag == 1
    ...
    for l = 1:2
        ...
        elseif l == 2
            plot(X1UB,CumD,'k','LineWidth',2,'DisplayName','???????????400???')
            hold on
            plot(TargetAlpha,SA(3,:,4),'r-','LineWidth',2,'DisplayName','??????800??')
        end
        hold off
        if l == 1
            xlabel('Output Loss (%)','FontSize',16)
