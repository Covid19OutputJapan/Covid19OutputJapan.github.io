function [CumD,GDPLoss,alphapath,SimData,SimN] = Covid_projection_control(InitialValues,alpha_on,alpha_off,th_on,th_off,beta,gamma,delta,V,h,k,POP0)
...
for i = 1:T
    alphapath(i) = alpha;
    SimN(i) = ((1 - h*(alpha/100))^k)*beta(i)*SimData(i,1)*SimData(i,2)*(1/POP0);
    ...
    SimData(i+1,4) = SimData(i,4) + delta(i)*SimData(i,2);
     if th_on <= ((1 - h*(alpha/100))^k)*beta(i)*SimData(i,1)*SimData(i,2)*(1/POP0)
        alpha = alpha_on;
    elseif th_off >= ((1 - h*(alpha/100))^k)*beta(i)*SimData(i,1)*SimData(i,2)*(1/POP0)
        alpha = alpha_off;
    end
end
